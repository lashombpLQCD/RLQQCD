...
! Define access to subroutines.
     ...
     private :: vev, average, averagePRIME, loopops, mulfor2,mulfor3,mulbac2,mulbac3, vv, vgv, momfacs, ppaverage2, ppaverage, ppaverage1,&
                ...
                printz2noise, nsaverage, nsaverage_abdou, newppaverage, newdoubleppaverage 
...
! and average. --AA
 ...
 subroutine generalaverage(Jave,numEV,numEVP,eigstart,eigstep,u,z2e,z2o,xe,xo,kappa, &
                           ...
                           newdoublexiinvPOLY,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,  &
                           iblv,rwdir,MRT,MRT2)
 ...
 real(kind=KR2),   intent(in),    dimension(:,:)         :: newxiinvPOLY,newdoublexiinvPOLY   
...
! integer(kind=KI), parameter                 :: nsub=6, nmom=5, nop=5, nsty=6
 integer(kind=KI), parameter                 :: nsub=6, nmom=5, nop=9, nsty=16
!BS integer(kind=KI), parameter                 :: nsub=6, nmom=5, nop=9, nsty=9
 ...
 real(kind=KR2),   dimension(2,nt,nmom,nop)           :: JaveNEWPP, JaveNEWDOUBLEPP
 real(kind=KR2),   dimension(2,nt,kmaxGMRES,nmom,nop) :: JaveNEWHFESPOLY, JaveNEWHFESDOUBLEPOLY   
...
!enddo
 ...
 JaveNEWDOUBLEPP = 0.0_KR2 
 JaveNEWHFESDOUBLEPOLY = 0.0_KR2 

 
 if (myid==0) then 
      print*, 'Line 210'
      print*, 'xe(1,1,1,1,1)', xe(1,1,1,1,1) 
      print*, 'xo(1,1,1,1,1)', xo(1,1,1,1,1) 
 endif 
...
JaveNEWPP = -JaveNEWPP  
 ...
 ! New double polynomial subtraction -PL 
 call newdoubleppaverage(JaveNEWDOUBLEPP,nmom,nop,momfac,u,z2e,z2o,ZERO_E,ZERO_O,kappa, &
              dobndry,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd, &
              iblv,rwdir,MRT,MRT2)!To use ppaverage,add MRT to the list

 JaveNEWDOUBLEPP = -JaveNEWDOUBLEPP  
...
! if (.true.) then 
 ...
 call eigaverage(JaveNEWHFESDOUBLEPOLY(:,:,:,:,:),numEVP,eigstart,eigstep,u,z2e,z2o,xe,xo,kappa(1),&
                 evecRevenP,evecRoddP,evecRevenP,evecRoddP,newdoublexiinvPOLY(:,:), &
                 coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT2,2_KI)
...
! end if 
 ...
 !if(myid==0) then 
 !    print*, 'JaveNS(1,1,imom,1)', JaveNS(1,1,1,1)
 !    print*, 'JaveNS(2,1,imom,1)', JaveNS(2,1,1,1)  
 !endif 
 ...
 do isub=1,nsub
   do eig=1,numEV_MAX
     ...
     ! =============== CHANGE BACK TO sty=1,14 !!!!!!!!!!!!!!!!!!!!!!!!!!======
     ! ========================================================================

     !do sty=1,10!BS changed from 9 to 12
     do sty=1,16 !Changed to 14 -PL 
      ...
      !BS else if (sty==11) then
       ...
       !sty = 15 only new double polysubtraction  -PL 
       else if (sty==15 .and. eig==1 .and. isub==1) then 
         Jave(:,:,isub,eig,sty,:) = JaveNS(:,:,imom,:) - JaveNEWDOUBLEPP(:,:,imom,:)
       !sty = 16 HFPOLY using new double polysubtraction  -PL 
       else if (sty==16 .and. isub==1) then 
         Jave(:,:,isub,eig,sty,:) = JaveNS(:,:,imom,:) - JaveHFES(:,:,eig,imom,:) &
                                  - ( JaveNEWDOUBLEPP(:,:,imom,:) - JaveNEWHFESDOUBLEPOLY(:,:,eig,imom,:) )
...
!BS   open(unit=31,file=trim(rwdir(myid+1))//"NSPSHFESPS.dat",status="old",     &
  ...
  !BS open(unit=32,file=trim(rwdir(myid+1))//"NSPSHFES.dat",status="old",     &
   ...
   ! Added newdoublepoly -PL 
   open(unit=44,file=trim(rwdir(myid+1))//"newdoublepp.dat",status="old",     &
        action="write",form="formatted",position="append")!PL
   open(unit=45,file=trim(rwdir(myid+1))//"newhfesdoublepoly.dat",status="old",     &
        action="write",form="formatted",position="append") !PL 
         ...
         do eig=1,numEV_MAX
           do it=1,nt
             ...
             if (sty==1 .and. eig==1 .and. isub==1) then
               ...
               !if (myid==0 .and. it==1) then 
               !    print*, 'Jtotal(1) = ', Jtotal(1,it,isub,eig,sty,iop), ' Jtotal(2)', Jtotal(2,it,isub,eig,sty,iop)  
               !endif 
            ...
            ! else if (sty==12 .and. ((eig == eigstart) .or. (eig > eigstart.and. 0 == mod(eig-eigstart,eigstep)) .or. (eig == numEVPRIME))) then
             ...
             ! Added by PL 2/9/21
               ...
               write(unit=44,fmt='(i3,i3,i3,i5,i3,f25.15,a3,f25.15)') sty,iop,isub,eig,it, &
                     Jtotal(1,it,isub,eig,sty,iop),'   ',Jtotal(2,it,isub,eig,sty,iop)!PL
             else if (sty==16 .and. isub==1) then
               write(unit=45,fmt='(i3,i3,i3,i5,i3,f25.15,a3,f25.15)') sty,iop,isub,eig,it, &
                     Jtotal(1,it,isub,eig,sty,iop),'   ',Jtotal(2,it,isub,eig,sty,iop)!PL
...
!BSprint *,"didnot print eig eigstart eigstep numEV ",eig,eigstart,eigstep,numEV
  ...
  ! close(unit=32,status="keep")!BS
   ...
   close(unit=44,status="keep") !PL
   close(unit=45,status="keep") !PL  
...
subroutine newxipolygenerator(newxiinvPOLY,numEV,eigstart,eigstep,u,kappa, &
 ...
 integer(kind=KI)                                :: iflag 




 ! SET DESIRED POLYNOMIAL DEGREE  
 real(kind=KR2),     dimension(2,1000)      :: rv ! Harmonic ritz values 
 real(kind=KR2),     dimension(2,1000)      :: lorv !Leja-ordered harmonic ritz values
...
!    integer(kind=KI)                                :: i ! index 
 ...
 !nsub = 6
  ...
  ! DON'T FORGET TO INITIALIZE POLYNOMIAL DEGREE SIZE IN DEFINITION FOR rv AND lorv 
...
!============================ Constructing Polynomial ===============================
    ...
    call gmresEIGritz(rwdir,z2e,z2o,xejunk,xojunk,paramsGMRES,u,GeeGooinv, &
   ...
   call genpoly(rwdir,lorv,p,z2e,z2o,MRT,ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,&
                    idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)
...
!========================== Applying the Polynomial ================================
 ...
 if (.true.) then
    ...
    call newpoly(sube,subo,be,bo,p,lorv,MRT, &
                 ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv, &
                 bc,nms,ib,lbd,ldiv,lvbc,0,MRT2,tEE,tOO)
...
subroutine newdoublexipolygenerator(newdoublexiinvPOLY,numEV,eigstart,eigstep,u,kappa, &
                        evecReven,evecRodd,evecLeven,evecLodd,z2e,z2o, &
                        coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,&
                        iblv,MRT2,gammaid,rwdir,MRT)
 ...
 real(kind=KR2),   intent(out), dimension(:,:)         :: newdoublexiinvPOLY
 integer(kind=KI), intent(in)                          :: numEV,gammaid
 integer(kind=KI), intent(in)                          :: eigstart,eigstep
 real(kind=KR),    intent(inout), dimension(:,:,:,:,:) :: u
 real(kind=KR),    intent(in), dimension(:)            :: kappa
 ...
 ! SET DESIRED POLYNOMIAL DEGREE  
 integer(kind=KI)                           :: p,p1,p2 ! Polynomial degrees 
 real(kind=KR2),     dimension(2,1000)      :: rv,rv1,rv2 ! Harmonic ritz values 
 real(kind=KR2),     dimension(2,1000)      :: lorv,lorv1,lorv2 !Leja-ordered harmonic ritz values
...
!    integer(kind=KI)                                :: i ! index 
 ...
 !nsub = 6
  ...
  if (myid==0) then 
       print*, "Inside newdoublexipolygenerator"
  endif 
...
!============================ Constructing Polynomial ===============================
   ...
   call gendoublepoly(rwdir,lorv1,lorv2,p1,p2,z2e,z2o,MRT,ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,&
                    idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)
...
!========================== Applying the Polynomial ================================
 ...
 if (.true.) then
    ...
    call newdoublepoly(sube,subo,be,bo,p1,lorv1,p2,lorv2,MRT, &
                       ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv, &
...
!==============Finish Dot Product For e*' (M'^-1 e) = e' (sub) = 1/xi' =================
      ...
      call vecdot(evecLeven(:,:,:,:,:,eig),sube,tmp1,MRT2)
      call vecdot(evecLodd(:,:,:,:,:,eig),subo,tmp2,MRT2)
      newdoublexiinvPOLY(:2, eig) = tmp1(:2) + tmp2(:2)
...
!    enddo
 ...
 end subroutine newdoublexipolygenerator
...
! integer(kind=KI), parameter :: nop=9, nsty=9, eigstart=1, eigstep=5, ksub=6
 integer(kind=KI), parameter :: nop=9, nsty=16, eigstart=1, eigstep=1, ksub=6!BS
! integer(kind=KI), parameter :: nop=9, nsty=10, eigstart=1, eigstep=5, ksub=6
 ...
 real(kind=KR2),  dimension(2,kmaxGMRES)              :: newxiinvPOLY,newdoublexiinvPOLY 
...
integer(kind=KI)   :: noiseunitnum
 ...
 ! For timing before Tr and after Tr calculation 
  integer(kind=KI)                            :: c1, c2, cr, cm
  real(kind=KR2)                              :: start_1, stop_1, start_2, stop_2, rate, start, finish
  ...
  ! Used for testing double polynomial method 

  real(kind=KR2),  dimension(6,ntotal,4,2,8)  :: xe2, xo2, xodiff, xediff 
  real(kind=KR2),  dimension(2)               :: tv, tv1, tv2
...
inquire(file=trim(rwdir(myid+1))//"newdoublepp.dat", exist=exists)
  if (.not. exists) then

open(unit=44,file=trim(rwdir(myid+1))//"newdoublepp.dat",status="new",     &
        action="write",form="formatted")
   close(unit=44,status="keep")
end if


inquire(file=trim(rwdir(myid+1))//"newhfesdoublepoly.dat", exist=exists)
  if (.not. exists) then

open(unit=45,file=trim(rwdir(myid+1))//"newhfesdoublepoly.dat",status="new",     &
        action="write",form="formatted")
  ...
  if(.false.) then
    do eig=1,numEVPRIME
     ...
     elseif(eig<100) then
      ...
      call printferm(be,bo,rwdir,noisefile,myid,iblv,vecblinv,MRT2,80)!BS printing 2/4/2016
...
call z4source(z2e,z2o,nvhalf,myid)
 ...
 ! Added for new polynomial method - PL 4/1/21 
 call newdoublexipolygenerator(newdoublexiinvPOLY,numEVPRIME,eigstart,eigstep,u,kappa,&
                               evectorRevnPRIME,evectorRoddPRIME,evectorRevnPRIME,evectorRoddPRIME,&
                               z2e,z2o,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT2,2_KI,rwdir,MRT)
...
!PL ************printing of newxiinvpoly starts********8888
...
if (.false.) then 
 ...
 if (myid==0) then
       ...
       inquire(file=trim(rwdir(myid+1))//"newxiinvpoly.dat", exist=exists)
           if (.not. exists) then
               open(unit=42,file=trim(rwdir(myid+1))//"newxiinvpoly.dat",status="new",&
                   action="write",form="formatted")
           ...
           open(unit=42,file=trim(rwdir(myid+1))//"newxiinvpoly.dat",status="old",&
                  action="write",form="formatted",position="append")
                        do eig =1,numEVPRIME
                            write(unit=42,fmt="(i5,a5,i5,a5,F17.10,a5,F17.10)")eig,"   ",isub,"   ",newxiinvPOLY(1,eig),"  ",newxiinvPOLY(2,eig)
                        enddo
...
!PL ************printing of newdoublexiinvpoly starts********8888
...
if (.false.) then 
 ...
 if (myid==0) then
       ...
       inquire(file=trim(rwdir(myid+1))//"newdoublexiinvpoly.dat", exist=exists)
           if (.not. exists) then
               open(unit=46,file=trim(rwdir(myid+1))//"newdoublexiinvpoly.dat",status="new",&
                   action="write",form="formatted")
           ...
           open(unit=46,file=trim(rwdir(myid+1))//"newdoublexiinvpoly.dat",status="old",&
                  action="write",form="formatted",position="append")
                        do eig =1,numEVPRIME
                            write(unit=46,fmt="(i5,a5,i5,a5,F17.10,a5,F17.10)")eig,"   ",isub,"   ",newdoublexiinvPOLY(1,eig),"  ",newdoublexiinvPOLY(2,eig)
                        enddo
...
! %%%%%%%%%%%%%%%%%% Write noise vectors to .LOG files  %%%%%%%%%%%%%%%%%%%%%
 ...
 !call printferm(z2e,z2o,rwdir,noisefile,myid,iblv,vecblinv,MRT2,80)!BS printing 2/4/2016


! %%%%%%%%%%%%%%%%% End of noise vectors output  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
!******************************end**************************!
   ...
   gxo = xoTMP(:,:,:,:,:,1)
    ...
    if (myid==0) then 
        print*, "Finished fermprop" 
    endif 


! %%%%%%%%%%%%%%%%%% Write gmresproj solution vectors gx to .LOG files  %%%%%%%%%%%%%%%%%%%%%

  if(.false.) then
     if(irhs<10) then
        write(gxsolfile,"(a7,i1)")"gxsoln-",irhs
     elseif(irhs<100) then
        write(gxsolfile,"(a7,i2)")"gxsoln-",irhs
     else
        write(gxsolfile,"(a7,i3)")"gxsoln-",irhs
     endif
         if (myid==0) then
             inquire(file=trim(rwdir(myid+1))//trim(gxsolfile)//".LOG", exist=exists)
             if (.not. exists) then
                open(unit=90,file=trim(rwdir(myid+1))//trim(gxsolfile)//".LOG",status="new",     &
        action="write",form="formatted")
 ...
 !call printferm(gxe,gxo,rwdir,gxsolfile,myid,iblv,vecblinv,MRT2,90)!BS printing 2/4/2016


! %%%%%%%%%%%%%%%%% End of solution vectors output  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (.false.) then 
    ...
    xe2(:6,:ntotal,:4,:2,:8) = 0.0_KR2
    xo2(:6,:ntotal,:4,:2,:8) = 0.0_KR2

    xediff(:6,:ntotal,:4,:2,:8) = 0.0_KR2
    xodiff(:6,:ntotal,:4,:2,:8) = 0.0_KR2


    !call newppgmresdr5EIG(rwdir,z2e,z2o,xe2,xo2,nGMRES,resmax,itermin,itercount,u,GeeGooinv,&
    !                      iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,&
    !                      lvbc,ib,lbd,iblv,MRT,MRT2)


    call newdoubleppgmresdrEIG(rwdir,z2e,z2o,xe2,xo2,nGMRES,resmax,itermin,itercount,u,GeeGooinv,&
                               iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,&
                               lvbc,ib,lbd,iblv,MRT,MRT2)


    !if (myid == 0) then 
    !    print *, "just done with ppgmresdr5EIG"
    !endif 


    !call cpu_time(time3) 

    !call newppminresdr5EIG(rwdir,z2e,z2o,xe2,xo2,nGMRES,resmax,itermin,itercount,u,GeeGooinv,&
    !                       iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,&
    !                       lvbc,ib,lbd,iblv,MRT,MRT2)
    ...
    if (myid == 0) then 
        print *, "just done with newppminresdr5EIG" 
    endif 
    ...
    call vecdot(xediff,xediff,tv1,MRT2)
    call vecdot(xodiff,xodiff,tv2,MRT2)
    tv = tv1 + tv2
    tv(2) = 0.0_KR2

    if (myid==0) then
    print*, "Loop over rhs's" 
    print*, "norm(x - px) = ", sqrt(tv(1)) 
    endif 
     ...
     call Hsingle(tmpO,u,xe2,idag,coact,bc,2_KI, &
                  vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT)

      call Hsingle(tmpE,u,xo2,idag,coact,bc,1_KI, &
                   vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT)

     tmpE(:6,:nvhalf,:4,:2,:8) = xe2(:6,:nvhalf,:4,:2,:8) - kappa(1)*tmpE(:6,:nvhalf,:4,:2,:8)
     tmpO(:6,:nvhalf,:4,:2,:8) = xo2(:6,:nvhalf,:4,:2,:8) - kappa(1)*tmpO(:6,:nvhalf,:4,:2,:8)
     ...
     tmp1 = tmp1 + tmp2
     call cmpxsqrt(tmp1,tmp2)
     if (myid==0) write(*,*) 'initial residual: ',tmp2(1)
...
!to do the g5M case,change z2 to gz2!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   call generalaverage(Jave,numEV,numEVPRIME,eigstart,eigstep,u,z2e,z2o,xeTMP(:,:,:,:,:,1),xoTMP(:,:,:,:,:,1),     &
                       ...
                       newdoublexiinvPOLY,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,rwdir,MRT,MRT2)
  ...
  if (myid==0) then 
      call cpu_time(stop_2) 
      call system_clock(c2) 
      finish = MPI_Wtime() 
      print*, "Finished irhs = ", irhs 
      print*, '("CPU time = ",f6.3," seconds")', stop_2 - start_2 
      print*, "system_clock : (c2-c1)/rate", (c2 - c1)/rate
      print*, "MPI_Wtime(): ", finish - start 
  endif 
 ...
 subroutine eigmodesofMprime(evectorRevnPRIME,evectorRoddPRIME,eigValExpandPRIME,numEVPRIME, &
   ...
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: xe, xo, xe2, xo2, xodiff, xediff
   ...
   ! Using to test preconditioning 

   real(kind=KR2), dimension(2)               :: tv, tv1, tv2
   real(kind=KR2)                             :: time1, time2, time3, time4
   real(kind=KR),  dimension(6,ntotal,4,2,8)  :: onesE, onesO 
   character(len=128) :: noisefile
...
if (myid==0) then
 ...
 ! Set to 1e-20_KR to speed up first RHS as up to 10th eval converges 
 ! and polynomial method saturates past 10th eval for 24242432 -PL  
 resmax = 1e-20_KR 


 if (myid==0) then 
     print*, "minresdr5EIG resmax = ", resmax 
 endif 
...
!    call newpoly(trye,tryo,deg,rv,ye,yo,MRT,ieo,ibleo,gblclr,&
...
!                     bc,nms,ib,lbd,ldiv,lvbc)
    ...
    if(myid==0) then 
        print*, "MPI_Wtime wall-time after minresdr5EIG: ", time2 - time1
    endif 


    if (myid == 0) then 
        print *, "just done with minresdr5EIG" 
    endif 


    !!! Testing Hermitian polynomial preconditioned gmres-dr 
    ...
    !call newdoubleppgmresdr5EIG(rwdir,z2e,z2o,xe2,xo2,nGMRES,resmax,itermin,itercount,u,GeeGooinv,&
    !                            iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,&
    !                            lvbc,ib,lbd,iblv,MRT,MRT2)
    ...
    if (myid == 0) then 
        print *, "just done with newdoubleppgmresdr5EIG" 
    endif 
...
!!!!!!!! ------------ call to minresdr inserted for testing!!!! - TW 8/10/18
   ...
   !if (myid ==0) then
   !          print *, "just done with minresdr5EIG" !PL 
...
!*****BS added for flipping gamma5 multiplication********
 ! gamma5*M
     do ieo=1,2
       do ibleo=1,8
         call gammamult(xe2, xo2, tmpE, tmpO, 5,ieo,ibleo)
       enddo
     enddo
...
! *********BS comment out ends here
     ...
     if (ii == 10) then
          print*, "|gamma5*M*x - eval*x| for 10th eval", tmp2(1)
     endif
...
! integer(kind=KI), parameter                         :: nmom=5, nop=5
 ...
 if (myid==0) then 
      print*, "Inside of nsaverage" 
 endif

 if (myid==0) then 
      print*, "xe(1,1,1,1,1) = ", xe(1,1,1,1,1) 
      print*, "xo(1,1,1,1,1) = ", xo(1,1,1,1,1) 
      print*, "xe(2,1,1,1,1) = ", xe(2,1,1,1,1) 
      print*, "xo(2,1,1,1,1) = ", xo(2,1,1,1,1) 
 endif 
...
!        For imom>1, iri=1..2 and iop=1..nop together label Im operators.
    ...
    real(kind=KR),    dimension(6,ntotal,4,2,8) :: sub1e, sub1o, sub2e, sub2o,&
                                                   vecsrc, temp, tue, tuo, &
                                                   sub3e, sub3o
    real(kind=KR),    dimension(6,nvhalf,4,2,8) :: sube, subo, subeTMP, suboTMP 
    integer(kind=KI), parameter                 :: itstep=2*nvhalf*npt/nt
    real(kind=KR),    dimension(18,itstep,2,8)  :: ubndry
    !real(kind=KR),    dimension(2,nvhalf,5)     :: Je, Jo !5 should be replaced by nop
    real(kind=KR),    dimension(2,nvhalf,nop)     :: Je, Jo !5 should be replaced by nop
    ...
    real(kind=KR2),     dimension(2)                :: stmp, setmp, sotmp 


    character(len=128) :: oldppfile 
    integer(kind=KI) :: irhs
    integer(kind=KI) :: exists
    integer(kind=KI) :: finished 

    real(kind=KR),    dimension(6,nvhalf,4,2,8) :: onesE, onesO
...
!Determine the polynomial for M.!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    do isite = 1,nvhalf
     vprime(:,isite,:,:,:,1) = be(:,isite,:,:,:)
     !vprime(:,isite,:,:,:,1) = onesE(:,isite,:,:,:)
    enddo !isite

    do isite = 1,nvhalf
     vprime1(:,isite,:,:,:,1) = bo(:,isite,:,:,:)
     !vprime1(:,isite,:,:,:,1) = onesO(:,isite,:,:,:)
    enddo !isite
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!7order!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (.false.) then 
         ...
         !gblclr = 2
         !call Hsingle(suboTMP,u,tmpEE,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
         !             nms,lvbc,ib,lbd,iblv,MRT)
         ! Next build H_eo * v_o.
         
         !gblclr = 1
         !call Hsingle(subeTMP,u,tmpOO,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
         !             nms,lvbc,ib,lbd,iblv,MRT)
         
         !subeTMP = tmpEE - kappa(1) * subeTMP
         !suboTMP = tmpOO - kappa(1) * suboTMP


        !subeTMP = vprimee - subeTMP 
        !suboTMP = vprimeo - suboTMP 
        ...
        call vecdot( subeTMP, subeTMP, setmp, MRT2 )
        call vecdot( suboTMP, suboTMP, sotmp, MRT2 ) 
        ...
        if (myid==0) then 
            ...
            print*, 'isub = , ||phi(A)b|| = ', isub, sqrt(stmp(1))   
        ...
        if( finished .ne. 1) then
           !if(irhs<10) then
           !   write(oldppfile,"(a6,i1)")"oldpp_noise-",irhs
           !elseif(irhs<100) then
           !   write(oldppfile,"(a6,i2)")"oldpp_noise-",irhs
           !else
           !   write(oldppfile,"(a6,i3)")"oldpp_noise-",irhs
           !endif

               write(oldppfile,"(a12,i1)") "oldpp_noise-",irhs 
               if (myid==0) then
                   inquire(file=trim(rwdir(myid+1))//trim(oldppfile)//".LOG", exist=exists)
                   if (.not. exists) then
                      open(unit=100,file=trim(rwdir(myid+1))//trim(oldppfile)//".LOG",status="new",     &
              action="write",form="formatted")
        ...
        call printferm(subeTMP,suboTMP,rwdir,oldppfile,myid,iblv,vecblinv,MRT2,100)!BS printing 2/4/2016
...
open(unit=80,file=trim(rwdir(myid+1))//trim(startingvector)//".LOG",status="new",&
 ...
 call printferm(be,bo,rwdir,startingvector,myid,iblv,vecblinv,MRT2,80)!BS 3/2/2016
...
!        For imom>1, iri=1..2 and iop=1..nop together label Im operators.
    ...
    real(kind=KR),    dimension(6,ntotal,4,2,8) :: sub1e, sub1o, sub2e, sub2o,&
                                                   vecsrc, temp, tue, tuo, &
                                                   sub3e, sub3o
    !real(kind=KR2),    dimension(6,nvhalf,4,2,8) :: sube, subo, subeTMP, suboTMP, tmpEE, tmpOO
    

    real(kind=KR),    dimension(6,ntotal,4,2,8) :: sube, subo, subeTMP, suboTMP, tmpEE, tmpOO
    ...
    real(kind=KR2), dimension(6,nvhalf,4,2,8)     :: vprimee,vprimeo
...
!============================ Constructing Polynomial ===============================
    ...
    !do isite = 1,nvhalf
        !vprime(:,isite,:,:,:,1) = be(:,isite,:,:,:)
        !vprimee(:,isite,:,:,:) = be(:,isite,:,:,:) 
    !    vprimee(:,isite,:,:,:) = onesE(:,isite,:,:,:) 
    !enddo !isite

    !do isite = 1,nvhalf
        !vprime1(:,isite,:,:,:,1) = bo(:,isite,:,:,:)
        !vprimeo(:,isite,:,:,:) = bo(:,isite,:,:,:) 
    !    vprimeo(:,isite,:,:,:) = onesO(:,isite,:,:,:) 
    !enddo !isite
    ...
    endif ! Falsed out old method 

    
    call genpoly(rwdir,lorv,p,be,bo,MRT,ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,&
                    idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)
...
!========================== Applying the Polynomial ================================
    ...
    if (.false.) then 
        ...
        !if( finished .ne. 1) then
           !if(irhs<10) then
           !   write(newppfile,"(a6,i1)")"newpp_noise-",irhs
           !elseif(irhs<100) then
           !   write(newppfile,"(a6,i2)")"newpp_noise-",irhs
           !else
           !   write(newppfile,"(a6,i3)")"newpp_noise-",irhs
           !endif
               write(preppfile,"(a12,i1)") "prepp_noise-",irhs 
               if (myid==0) then
                   inquire(file=trim(rwdir(myid+1))//trim(preppfile)//".LOG", exist=exists)
                   if (.not. exists) then
                      open(unit=80,file=trim(rwdir(myid+1))//trim(preppfile)//".LOG",status="new",     &
              action="write",form="formatted")
                      close(unit=80,status="keep")
                   endif
              endif
        ...
        ! change be,bo to onesE,onesO
        call printferm(onesE,onesO,rwdir,preppfile,myid,iblv,vecblinv,MRT2,80)!BS printing 2/4/2016
    ...
    call newpoly(subeTMP,suboTMP,be,bo,p,lorv,MRT, &
                 ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv, &
                 bc,nms,ib,lbd,ldiv,lvbc,0,MRT2,tEE,tOO)
    ...
    if (.false.) then 
       ...
       ! if( finished .ne. 1) then
           ...
           !   write(newppfile,"(a6,i3)")"newpp_noise-",irhs
           !endif
               write(newppfile,"(a12,i1)") "newpp_noise-",irhs 
               if (myid==0) then
                   inquire(file=trim(rwdir(myid+1))//trim(newppfile)//".LOG", exist=exists)
                   if (.not. exists) then
                      open(unit=81,file=trim(rwdir(myid+1))//trim(newppfile)//".LOG",status="new",     &
              action="write",form="formatted")
        ...
        call printferm(subeTMP,suboTMP,rwdir,newppfile,myid,iblv,vecblinv,MRT2,81)!BS printing 2/4/2016
    ...
    !do ieo=1,2 
    !   do ibleo=1,8
    !       call gammamult( subeTMP, suboTMP, sube, subo, 5, ieo, ibleo ) 
    !   enddo 
    !enddo  
...
!========================= Calculating the Argument of Sum ==========================
    ...
    do ibleo = 1,8
        do ieo = 1,2
            call loopops(1_KI,sub1e,sub1o,u,be,bo,Je,Jo,ieo,ibleo,bc,vecbl, &
                         vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT)
            call spacesum(Jtime(:,:,:,:),Je,Jo,ieo,ibleo,momfac,nmom,nop, &
                          myid,vecbl,vecblinv,MRT)
        enddo ! ieo
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 subroutine newdoubleppaverage(Jtime,nmom,nop,momfac,u,be,bo,xe,xo,kappa,dobndry, &
                    coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd, &
                    iblv,rwdir,MRT,MRT2)
! This is polynomial subtraction using new poly method -PL 
...
!        For imom>1, iri=1..2 and iop=1..nop together label Im operators.
    ...
    ! SET TO DESIRED POLYNOMIAL DEGREE 
    integer(kind=KI)                           :: p,p1,p2          ! Polynomial degree
    real(kind=KR2),     dimension(2,1000)      :: rv,rv1,rv2       ! Harmonic ritz values 
    real(kind=KR2),     dimension(2,1000)      :: lorv,lorv1,lorv2 !Leja-ordered harmonic ritz values
...
!============================= Initializations ==================================
  ...
  ! DON'T FORGET TO INITIALIZE POLYNOMIAL DEGREE SIZE IN DEFINITION FOR rv AND lorv 
    ...
    if (myid==0) then 
            print*, 'In newdoubleppaverage'
    endif 


    if (myid==0) then  
            print*, 'Double polynomial degree p1:        ', p1
            print*, 'Double polynomial degree p2:        ', p2 
    endif
...
!============================ Constructing Polynomial ===============================
    ...
    if (myid==0) then 
         print*, "Inside newdoubleppaverage" 
    endif 
    ...
    call gendoublepoly(rwdir,lorv1,lorv2,p1,p2,be,bo,MRT,ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,&
                    idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)
...
!========================== Applying the Polynomial ================================
    ...
    ! change be,bo to onesE,onesO
    ! Had 'params(1)' instead of p? -PL 
    call newdoublepoly(subeTMP,suboTMP,be,bo,p1,lorv1,p2,lorv2,MRT, &
                       ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv, &
                       bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)
...
!========================= Calculating the Argument of Sum ==========================
    ...
    Jtime(:,:,:,3) = kappa(1)*Jtime(:,:,:,3)
    Jtime(:,:,:,4) = kappa(1)*Jtime(:,:,:,4)
    end subroutine newdoubleppaverage

...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 real(kind=KR2), dimension(:,:,:,:,:,:), intent(in) :: evecReven, evecRodd
 real(kind=KR2), dimension(:,:,:,:,:,:), intent(in) :: evecLeven, evecLodd
 real(kind=KR2), dimension(:,:,:,:,:,:), intent(in) :: evecRevenP, evecRoddP
 real(kind=KR2), dimension(:,:),       intent(in)   :: scalarMultiplier,scalarMultiplierP
 real(kind=KR2), dimension(:,:,:),       intent(in) :: xiinvscalarMultiplier,xiinvscalarMultiplierPRIME 
 ...
 real(kind=KR2),   dimension(2,nt,nsub,nmom,nop)          :: Javepert
 real(kind=KR),    dimension(2,nt,kmaxGMRES,nsty,nmom,nop)   :: Javeeig
 real(kind=KR),    dimension(2,nt,kmaxGMRES,nsty,nmom,nop)   :: JaveeigCor

 real(kind=KR), dimension(2,ntotal,4,2,8)  :: ZERO_E, ZERO_O

 integer(kind=KI) :: isub,eig,sty,iop,isty,it
 ...
 ! eig subtraction and NS
 call eigaverage(Javeeig,numEV,eigstart,eigstep,u,z2e,z2o,xe,xo,kappa, &
                 evecReven,evecRodd,evecLeven,evecLodd,scalarMultiplier, &
                 coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd, &
                 iblv,MRT2,1_KI)

 ! copy data to final storage (NS only)
 do isub = 1,1 
   do isty=1,1
     do eig=1,1 
       do iop=1,nop
         Jave(:2,:nt,iop,isty,eig,isub) = Javeeig(:2,:nt,eig,isty,1,iop)
       enddo
     enddo
   enddo
 enddo

 ! copy data to final storage (ES only)
 do isub = 1,1 
   do isty=3,3
     do eig=1,numEV 
       do iop=1,nop
 ...
 call eigaverage(Javeeig,numEVP,eigstart,eigstep,u,z2e,z2o,xe,xo,kappa, &
                 evecRevenP,evecRoddP,evecRevenP,evecRoddP,scalarMultiplierP, &
                 coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd, &
                 iblv,MRT2,2_KI)


 ! copy data to final storage (HFES only)
 do isub = 1,1 
   do isty=4,4
     do eig=1,numEVP 
       do iop=1,nop
 ...
 ! pert subtraction
 ! inorder to account for the "different" soluction vector x' => gamma5Mx' = z
 ...
 call average(Javepert,nsub,nmom,nop,momfac,u,z2e,z2o,ZERO_E,ZERO_O,kappa(1),dobndry, &
              coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd, &
              iblv,rwdir,MRT2)

 ! copy data to final storage (PS)
 do isty = 2,2
   do eig = 1,1
     do isub=2,nsub
       do iop=1,nop
         Jave(:2,:nt,iop,isty,eig,isub) = Jave(:2,:nt,iop,1,1,1) + Javepert(:2,:nt,isub,1,iop)
if (myid==0) write(*,*) isub, Javepert(:,1,isub,1,5)
       enddo
     enddo
   enddo
 enddo
      ...
      integer(kind=KI) :: eig,sty,iop,it,isub
      integer(kind=KI), parameter :: ksub = 6, nop=5

      if(myid==0) then

     ! ***** Output file for Non-Subtraction *****
        open(unit=20,file=trim(rwdir(myid+1))//"ns.dat",status="old", &
        ...
        do sty=1,1
          do iop=1,nop
            do isub=1,1
              do eig=1,1
                do it=1,nt
                  write(unit=20,fmt='(i3,i3,i3,i5,i3,f25.15,a3,f25.15)') sty,iop,isub,eig,it, &
                               Jtotal(1,it,iop,sty,eig,isub),'   ',Jtotal(2,it,iop,sty,eig,isub)
                enddo
              enddo
            enddo
          enddo
        enddo
        write(unit=20,fmt='(a7)') '-------'

        close(unit=20,status="keep")


     ! ***** Output file for Eigenspectrum Subtraction *****
        open(unit=21,file=trim(rwdir(myid+1))//"es.dat",status="old", &
             action="write",form="formatted",position="append")
        ...
        do sty=3,3
          ...
          enddo
        enddo
        write(unit=21,fmt='(a7)') '-------'

        close(unit=21,status="keep")

     ! ***** Output file for Hermitian Forced Eigenspectrum Subtraction *****
        ...
        do sty=4,4
          do iop=1,nop
            do isub=1,1
              do eig=1,numEVPRIME
                do it=1,nt
                  if (eig==eigstart .or. (eig>eigstart .and. 0==mod(eig-eigstart,eigstep)) .or. eig == numEVPRIME) then
          ...
          enddo
        enddo
        write(unit=24,fmt='(a7)') '-------'

        close(unit=21,status="keep")


     ! ***** Output file for Perturbative Subtraction *****
        ...
        do sty=2,2
          do iop=1,nop
            do isub=3,5,2
              do eig=1,1
                do it=1,nt
                  write(unit=22,fmt='(i3,i3,i3,i5,i3,f25.15,a3,f25.15)') sty,iop,isub+1,eig,it, &
                               Jtotal(1,it,iop,sty,eig,isub),'   ',Jtotal(2,it,iop,sty,eig,isub)
                enddo
              enddo
            enddo
          enddo
        enddo
        write(unit=22,fmt='(a7)') '-------'

        close(unit=22,status="keep")


if (.false.) then
     ! ***** Output file for ES-PS Combo Subtraction *****
        open(unit=23,file=trim(rwdir(myid+1))//"espsc.dat",status="old",&
             action="write",form="formatted",position="append")
          ...
          do iop=1,nop
            do isub=3,5,2
              do eig=1,numEV
                do it=1,nt
                  if (eig==eigstart .or. (eig>eigstart .and. 0==mod(eig-eigstart,eigstep)) .or. eig == numEV) then
                    write(unit=23,fmt='(i3,i3,i3,i5,i3,f25.15,a3,f25.15)') sty,iop,isub+1,eig,it, &
                                 Jtotal(1,it,iop,sty,eig,isub),'   ',Jtotal(2,it,iop,sty,eig,isub)
                  endif
                enddo
              enddo
...
if (myid == 0) then
 ...
 subroutine testFUNC(u,kappa,nrhs,coact,bc,vecbl,vecblinv,myid,ntmqcd,nn,ldiv, &
   ...
   integer(kind=KI)                             :: idag, ierr, runs
   real(kind=KR2),  dimension(2)                :: dottmp1,dottmp2
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: b,tmpE,bepart, tmpO
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: xe,   xo
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: gxe, gxo
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: z2e, z2o
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: gz2e, gz2o
   ...
   integer(kind=KI), parameter :: nop=5, nsty=6, eigstart=1, eigstep=10, ksub=6
   ...
   real(kind=KR2),   dimension(2,nt,nop,nsty,kmaxGMRES,ksub)   :: Jvev,Jave,Jtotal
   
   if (myid==0) write(*,*), 'here-testFUNC'
   ...
   ! GMRES Parameters
   nGMRES(1) = 60 ! must be smaller than what is used in eigmodesofM
   nGMRES(2) = 40 ! must be smaller than what is used in eigmodesofM
   itermin = 10
   iflag = -1  ! Wilson
   GeeGooinv = 0.0_KR2
   resmax = 1e-7_KR2
   inverter = 6 ! use gmresdr/gmresproj
   isignal = 2  ! used to identify gmresproj
   mvp = 0
   gdr = 0.0_KR2
   ...
   do runs=1,200
     ...
     call generalaverage(Jave,numEV,numEVPRIME,eigstart,eigstep,u,z2e,z2o,xeTMP(:,:,:,:,:,1),xoTMP(:,:,:,:,:,1), &
                         ...
                         evectorRevnPRIME, evectorRoddPRIME, inv_lamPRIME, &
                         xiinv, xiinvPRIME, &
                         coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd, &
                         iblv,rwdir,MRT2)
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 ...
 !                  u,rwdir,coact,nms,bc,nn,iblv,ldiv,lvbc,ib,lbd,myid,MRT,MRT2,numprocs)
   ...
   enddo ! full runs
      ...
      ! OUTPUT:
      !   Jtime(iri,it,eig,sty,iop) are the averaged operators.
      ...
      real(kind=KR2), dimension(:,:,:,:,:,:), intent(in) :: evecReven, evecRodd
      real(kind=KR2), dimension(:,:,:,:,:,:), intent(in) :: evecLeven, evecLodd
      real(kind=KR2), dimension(:,:),       intent(in)   :: scalarMultiplier
      real(kind=KR),    intent(in),    dimension(:,:,:)     :: coact
      integer(kind=KI), intent(in),    dimension(:)         :: bc, nms
      integer(kind=KI), intent(in),    dimension(:,:)       :: vecbl, vecblinv
      integer(kind=KI), intent(in),    dimension(:,:)       :: nn, iblv
      ...
      real(kind=KR2), dimension(6,nvhalf,4)     :: temp

      integer(kind=KI), parameter                      ::  nmom=5, nop=5
      real(kind=KR2),    dimension(2,nvhalf,nop)        :: Je, Jo
      real(kind=KR2),    dimension(2,nvhalf,nop)        :: gJe, gJo
      ...
      integer(kind=KI)        :: isite, iri, ibl, itbit
      ...
      sty = 1 ! 1 represents non-subtraction
      eig = 1 ! 1 is only value of eig used when sty=1 non-subtraction
      do ibleo = 1,8
        do ieo = 1,2
          call eigloopops(0_KI,xe,xo,u,z2e,z2o,Je,Jo,ieo,ibleo,bc,vecbl, &
                       vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT2)
          call spacesum(Jtime(:,:,eig,sty,:,:),Je,Jo,ieo,ibleo,momfac,nmom,nop, &
      ...
      do eig=1,numEV ! only do small modes
        ...
        ! tmp1 = (1/lam) * [L^dag (dot) z2] = scalarMultiplier(:2,eigindx) * tmp2
        ...
        if ( (eig == eigstart) .or. (eig > eigstart .and. 0 == mod(eig-eigstart,eigstep)) .or. (eig == numEV) ) then
          ...
          if (ESHFES  == 1) then ! ES
            sty = 3 ! Eigenspectrum Subtraction
            ! apply operators, via time slices
            do ibleo = 1,8
              do ieo = 1,2
            ...
            sty = 4 ! Hermitian Forced Eigenspectrum Subtraction
            ! apply operators, via time slices
            do ibleo = 1,8
              do ieo = 1,2
      ...
      do eig=1,numEV
        Jtime(:,:,eig,sty,:,1) = kappa(1)*(Jtime(:,:,1,1,:,1) - Jtime(:,:,eig,sty,:,1))
        Jtime(:,:,eig,sty,:,2) = kappa(1)*(Jtime(:,:,1,1,:,2) - Jtime(:,:,eig,sty,:,2))
        Jtime(:,:,eig,sty,:,3) = kappa(1)*(Jtime(:,:,1,1,:,3) - Jtime(:,:,eig,sty,:,3))
        Jtime(:,:,eig,sty,:,4) = kappa(1)*(Jtime(:,:,1,1,:,4) - Jtime(:,:,eig,sty,:,4))
        Jtime(:,:,eig,sty,:,5) = Jtime(:,:,1,1,:,5) - Jtime(:,:,eig,sty,:,5)
      enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     integer(kind=KI), parameter                         :: nsub=6, nmom=5, nop=5
     integer(kind=KI)                                    :: is
     ...
     real(kind=KR),    dimension(6,nvhalf,4,2,8,1)       :: xo
...
!        For imom>1, iri=1..2 and iop=1..nop together label Im operators.
    ...
    integer(kind=KI), parameter                 :: itstep=2*nvhalf*npt/nt
    real(kind=KR),    dimension(18,itstep,2,8)  :: ubndry
    real(kind=KR),    dimension(2,nvhalf,5)     :: Je, Jo
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     !  ! construct rhs for reduced problem: b=z2e/(k^2)+Heo*z2o/k
     !  tmpO(:6,:ntotal,:4,:2,:8) = z2o(:6,:ntotal,:4,:2,:8)
     !  call Hsingle(tmpE,u,tmpO,idag,coact,bc,1_KR,vecbl,vecblinv, &
     !               myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT)
     !  b(:6,:nvhalf,:4,:2,:8) = (1.0_KR2/(kappa(1) * kappa(1))) * z2e(:6,:nvhalf,:4,:2,:8) &
     !                          +(1.0_KR2/kappa(1)) * tmpE(:6,:nvhalf,:4,:2,:8)
     !
     ...
     !               lvbc,ib,lbd,iblv,MRT,MRT2)
     !
     !  ! solve odd solution vector: xo = z2o + k*Hoe*xe
     !  tmpE(:6,:ntotal,:4,:2,:8) = xe(:6,:ntotal,:4,:2,:8)
     !  call Hsingle(tmpO,u,tmpE,idag,coact,bc,2_KR,vecbl,vecblinv, &
     !               myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT)
     !  xo(:6,:nvhalf,:4,:2,:8)=z2o(:6,:nvhalf,:4,:2,:8)+kappa(1)*tmpO(:6,:nvhalf,:4,:2,:8)

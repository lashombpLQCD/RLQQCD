...
! How to specify strings of undetermined length? Are these right?  
    ...
    real(kind=KR2), dimension(6,ntotal,4,2,8) :: dphie,dphio,h,r0,r0e,r0o,we,wo,&
                                                 ...
                                                 workEE,workOO,tempE,tempO,wkE,wkO
...
!------------------------------parameters for the polynomial-------------------
    ...
    real(kind=KR2),     dimension(6,ntotal,4,2,8,1)   :: trye,tryo,ye,yo
    integer(kind=KI)                                :: p
    real(kind=KR2),     dimension(2,nmaxGMRES+1)    :: c,c2                                               
...
!------------------------------------------------------------------------------
   ...
   !------------- I originally commented this out
   ...
   !------------- - I originally commented this out end
...
!!!!!!!!!!This will need a call to the function that returns y = P(A)*r (applyppgamma5)
    ...
    !    v(:,k,:,:,:,1) = y(:,k,:,:,:,1) !ORIGINAL -PL 
        ve(:,k,:,:,:,1) = trye(:,k,:,:,:,1) ! Use y=P(A)*r to generate V_(m+1)&Hbar_m
        vo(:,k,:,:,:,1) = tryo(:,k,:,:,:,1) 
    enddo!k
...
!!!!!!!!!!!!!!! What I had originally but decided to rename for consistency 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !call vecdot(ve(:,:,:,:,:,1),ve(:,:,:,:,:,1),tv1,MRT2)
    !call vecdot(vo(:,:,:,:,:,1),vo(:,:,:,:,:,1),tv2,MRT2)
...
! For use in Morgan's step 2a, define c = beta*e_1.
!!! using 'tv' instead of 'beta' to be consistent with minresdr5EIG subroutine -
!PL 
!    c(1,1) = beta(1)

!!!! Don't think these are necessary. c_m is needed for solving least squares
!and comes from c_(m+1) = V_(k+1) * r_0. In the section on solving the least
!squares problem in this subroutine, V_(k+1) * r_0 is calculated explicitly
!rather than storing the paramater. Might consider changing in the future for fewer dot
!products. 
!    c(1,1) = tv(1) 
!    c(2,1) = 0.0_KR2
!    c2(:,1) = c(:,1)
...
!!!!!END OF INVERTERS CODE
    ...
    !NEED TO USE PRECONDITIONER FILE HERE TO APPLY PRECONDITIONER WITH GAMMA MULT
      ...
      !!!!Apply the matrix, i.e. w = Mgamma5 * y = Mgamma5 * p(Mgamma5)z
...
if (.false.) then
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==50 .OR. icycle==100 .OR. icycle==200 .OR. icycle==250 .OR. icycle==1000 .OR. icycle==2000) then
   ...
   !!!!!!!!Comment out one of the following do loops to update solution -PL 


   !Use this this do loop to update solution with right preconditioner -PL
   do i=1,mdr
    do isite=1,nvhalf

     tempE(:,isite,:,:,:) = ym(i) * ve(:,isite,:,:,:,i)
     tempO(:,isite,:,:,:) = ym(i) * vo(:,isite,:,:,:,i) 

     call applyppgamma5(tempE,tempO,p,co,wkE,wkO,MRT,ieo,ibleo,gblclr,&
                        kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                        bc,nms,ib,lbd,ldiv,lvbc)

     xe(:,isite,:,:,:)=xe(:,isite,:,:,:)+wkE(:,isite,:,:,:)
     xo(:,isite,:,:,:)=xo(:,isite,:,:,:)+wkO(:,isite,:,:,:)
    enddo !isite
   enddo !i



   !Use this do loop to update solution without right preconditioner -PL
   !Update the solution and compute the residual norm
   !do i=1,mdr
   ! do isite=1,nvhalf

     !Ignore this line when uncommenting
...
!   FUTURE VERSIONS:
!
!       Currently only able to handle left preconditioning. This will be
!       extended to right preconditioning in the future.
!
...
real(kind=KR),         intent(out),    dimension(6,ntotal,4,2,8,1) :: ye,yo
!real(kind=KR),                         dimension(6,ntotal,4,2,8,1)   :: re,ro
integer(kind=KI),      intent(in)                                  :: p
real(kind=KR),                         dimension(6,ntotal,4,2,8,p) :: trye,tryo
real(kind=KR2),        intent(in),     dimension(2,p)              :: co
integer(kind=KI),      intent(in)                                  :: MRT
real(kind=KR2),                        dimension(6,ntotal,4,2,8,1) :: tmpEE,tmpOO,&
                                                                      we,wo
integer(kind=KI)                                                   :: ieo,ibleo,&
                                                                      gblclr,i,k,&
                                                                      icri
...
!!!Have input such that it takes in both even and odd vectors and preconditions
!!!both simultaneously
    ...
    do icri=1,5,2
        do k=1,nvhalf
            ...
            ye(icri,k,:,:,:,1)   = co(1,1)*trye(icri,k,:,:,:,1) &
                                  -co(2,1)*trye(icri+1,k,:,:,:,1)
            ye(icri+1,k,:,:,:,1) = co(1,1)*trye(icri+1,k,:,:,:,1) &
                                  +co(2,1)*trye(icri,k,:,:,:,1)

            yo(icri,k,:,:,:,1)   = co(1,1)*tryo(icri,k,:,:,:,1) &
                                  -co(2,1)*tryo(icri+1,k,:,:,:,1)
            yo(icri+1,k,:,:,:,1) = co(1,1)*tryo(icri+1,k,:,:,:,1) &
        ...
        do ieo=1,2
            do ibleo=1,8
                call gammamult(trye(:,:,:,:,:,i),tryo(:,:,:,:,:,i),&
                               tmpEE(:,:,:,:,:,1),tmpOO(:,:,:,:,:,1),5,ieo,ibleo)
            enddo !ibleo
        enddo !ieo

        gblclr = 2

        call Hsingle(wo(:,:,:,:,:,1),u,tmpEE(:,:,:,:,:,1),idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                     nms,lvbc,ib,lbd,iblv,MRT)

        gblclr = 1

        call Hsingle(we(:,:,:,:,:,1),u,tmpOO(:,:,:,:,:,1),idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                     nms,lvbc,ib,lbd,iblv,MRT)

...
!AGAINST THEMSELVES? I.E. DO YOU NEED ONE OR TWO NORMALIZATIONS CONSTANTS? - PL
    ...
    !call vecdot(ve(:,:,:,:,:,1),ve(:,:,:,:,:,1),betao,MRT2)
...
!!!!!END OF INVERTERS CODE
    ...
    !NEED TO USE PRECONDITIONER FILE HERE TO APPLY PRECONDITIONER WITH GAMMA MULT
     ...
     ! gblclr = 2
     ! call Hsingle(workOO,u,tmpEE,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
     !              nms,lvbc,ib,lbd,iblv,MRT)

     ! gblclr = 1
     ! call Hsingle(workEE,u,tmpOO,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                   nms,lvbc,ib,lbd,iblv,MRT)

     ! workEE = tmpEE - kappa(1) * workEE
     ! workOO = tmpOO - kappa(1) * workOO
...
real(kind=KR2),                         dimension(2,p,1)            :: cls,clsE,&
                                                                       clsO
real(kind=KR2),                         dimension(2,p)              :: co,coE,coO

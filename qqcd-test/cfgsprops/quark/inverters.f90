...
! Define access to subroutines.
    public  :: bicgstab, m3r, cgne, qmr, qmrgam5, gmres, ppgmresproject, &
               ...
               vecdot, vecdot1, ppminresdr5EIG, minresdr5EIG, minresdr, & 
               newppgmresdr5EIG, newdoubleppgmresdr5EIG, newppminresdr5EIG, & 
               newdoubleppgmresdrEIG 
...
if (.true.) then !to get residual norm of linear equations -TW 11/14/18
...
! NOTE ~ BIG NOTE - might need to pass in idag into psc if that part of gamma5mult is needed
    ...
    kDR = GMRES(2)
      ...
      !if (myid==0) then 
      !     print*, 'isignal= ', isignal 
      !     print*, 'Line 4690 in inverters.f90'
      !endif

    if (isignal == 1) then

        !if (myid==0) then 
        !    print*, 'Inside isignal == 1'
        !endif
      ...
      !if (myid==0) then 
      !     print*, 'In isignal == 1'
      !     print*, 'Line 4705 in inverters.f90'
      !     print*, 'xshift(1,1,1,1,1,1)', xshift(1,1,1,1,1,1) 
      !     print*, 'xshift(2,1,1,1,1,1)', xshift(2,1,1,1,1,1) 
      !endif
...
!     gmres(m-k) - proj(k)
    ...
    if (myid==0) then 
        print*, 'GMRESproj(1)= ', GMRESproj(1) 
        print*, 'GMRESproj(2)= ', GMRESproj(2) 
    endif 
...
!     GMRESproj(2) = 100
    ...
    x(:,:,:,:,:) = xshift(:,:,:,:,:,1)
      !if (myid==0) then 
      !     print*, 'Outisde of isignal'
      !     print*, 'Line 4737 in inverters.f90'
      !     print*, 'xshift(1,1,1,1,1,1)', xshift(1,1,1,1,1,1) 
      !     print*, 'xshift(2,1,1,1,1,1)', xshift(2,1,1,1,1,1) 
      !endif
...
!    integer(kind=KI), dimension(2)                          :: ipiv2!order of polynomial
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! NOTE ~ I don't think that I need this ....
    ...
    !if (myid==0) then 
    !    print*, 'Line 7732 in inverters.f90'
    !    print*, 'rshift(1,1,1,1,1,1)= ', rshift(1,1,1,1,1,1)
    !    print*, 'rshift(2,1,1,1,1,1)= ', rshift(2,1,1,1,1,1)  
    !endif 
...
! with something other than zeros.
    ...
    !if (myid==0) then 
    !    print*, 'Line 7795 in inverters.f90'
    !    print*, 'rshift(1,1,1,1,1,2)= ', rshift(1,1,1,1,1,2)
    !    print*, 'rshift(2,1,1,1,1,2)= ', rshift(2,1,1,1,1,2)  
    !endif 
...
!         do is = 1,nshifts
          is=1
             ...
             !if (myid==0) then 
             !    print*, 'Line 8489 in inverters.f90'
             !    print*, 'xshift(1,1,1,1,1,1)= ', xshift(1,1,1,1,1,1)
             !    print*, 'xshift(2,1,1,1,1,1)= ', xshift(2,1,1,1,1,1)  
             !    print*, 'xb(1,1,1,1,1)= ', xb(1,1,1,1,1)
             !    print*, 'xb(2,1,1,1,1)= ', xb(2,1,1,1,1)  
             !endif 
         ...
         rn = betashift(1,1)
          ...
          ! if (j >= nDR) then 
             if (myid==0) then
               open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
                     form="formatted",status="old",position="append")
                         write(unit=8,fmt=*) "gmresdrproject-gdr",mvp,rn/rinit  !BS 4/3/2
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 subroutine ppminresdr5EIG(rwdir,be,bo,xe,xo,ldr,resmax,itermin,itercount,u,GeeGooinv, &
                           iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                           lvbc,ib,lbd,iblv,MRT,MRT2)
!==========================================================================
! A polynomial preconditioned variant of the inverter MinRes-DR(mdr,kdr) 
! that solves the non-hermetian problem M*x=phi by converting it into a 
! hermetain problem. For Wilson fermions this can be done through
! multiplication with gamma5 and solving gamma5*M*x=gamma5*phi. For all 
! cases we can use M^dagger*M*x=M^dagger*phi.
! NOT WORKING!!!!!!!
!==========================================================================
!  INPUT:
!   rwdir   	= the read/write directory 
!   A       	= an nxn symmetric matrix
!   phi()   	= a single source vector 
...
!   basis   	= the basis used in constructing polynomial preconditioner
!   lr      	= string that indicates choice of a left or right
!                 preconditioner
!
!--------------------------------------------------------------------------
!  OUTPUT:
!   x       	= computed solution vector 
!   itercount 	= the number of iterations used by landr.
!--------------------------------------------------------------------------
    ...
    integer(kind=KI),   intent(in),     dimension(:,:,:,:)      :: ib
    logical,            intent(in),     dimension(:,:)          :: lbd
! How to specify strings of undetermined length? Are these right?  
...
!------------------------------parameters for the polynomial-------------------
    ...
    real(kind=KR2),     dimension(2)                :: betae,betao,beta
    real(kind=KR2),     dimension(6,ntotal,4,2,8)   :: trye,tryo,ye,yo
    integer(kind=KI)                                :: p
    real(kind=KR2),     dimension(2,nmaxGMRES+1)    :: c,c2                                               

    ! mdr = ldr(1) 
    real(kind=KR2),     dimension(6,ntotal,4,2,8) :: TTmpe, TTmpo, xTMPe, xTMPo    
    real(kind=KR2),     dimension(2,nmaxGMRES)    :: rv, lorv

!counter
    integer(kind=KI)                                :: k 
    integer(kind=KI),   dimension(2)                :: paramsGMRES
    !real(kind=KR2),     dimension(6,ntotal,4,2,8,mdr+1) :: vprimee, vprimeo
!------------------------------------------------------------------------------
    ...
    !----------Create polynomial preconditioner here and implement-----
    ...
    normnum_hermetian=sqrt(tv(1)) 
    xrn(1) = sqrt(tv(1))  
                          ! this value will be output to CFGSPROPS.LOG
    ...
    !CODE FROM PPGMRESDR FOR INITIALIZATIONS OF POWER BASIS


!!!!! THIS ALL SHOULD BE INITIALIZATIONS IN THE MINRESDR5EIG FILE



!!!!!NEED TO CONSTRUCT PRECONDITIONER HERE


    p = 7 !polynomial degree

    !call powerbasis(be,bo,p,co,MRT,MRT2,ieo,ibleo,gblclr,kappa,u,coact,&
    !                vecbl,vecblinv,idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc)


!!!!PRINTING OUT COEFFICIENTS TO TEST
...
!!!!!!!!!!This will need a call to the function that returns y = P(A)*r (applyppgamma5)

    !call applyppgamma5(trye,tryo,p,co,ye,yo,MRT,ieo,ibleo,gblclr,&
    ...
    call gmres5EIGritz(rwdir,be,bo,xe,xo,paramsGMRES,u,GeeGooinv, &
                       iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
    ...
    !call newpoly(trye,tryo,p,lorv,ye,yo,MRT,ieo,ibleo,gblclr,&
    !             kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
    !             bc,nms,ib,lbd,ldiv,lvbc)

 
    !do k=1,nvhalf
    !    v(:,k,:,:,:,1) = y(:,k,:,:,:,1) !ORIGINAL -PL 
    !    ve(:,k,:,:,:,1) = trye(:,k,:,:,:) ! Use y=P(A)*r to generate V_(m+1)&Hbar_m
    !    vo(:,k,:,:,:,1) = tryo(:,k,:,:,:) 
    !enddo!k
...
!!NEED BETA FOR EACH EVEN AND ODD DOT PRODUCTS AND ADD TOGETHER TO GET SINGLE
!BETA 

!!QUESTION: DO YOU NEED TO NORMALIZE AGAINST FULL VECTOR OR BOTH EVEN AND ODD
!AGAINST THEMSELVES? I.E. DO YOU NEED ONE OR TWO NORMALIZATIONS CONSTANTS? - PL


    !call vecdot(v(:,:,:,:,:,1),v(:,:,:,:,:,1),beta,MRT2) !ORIGINAL -PL 
    call vecdot(ve(:,:,:,:,:,1),ve(:,:,:,:,:,1),betae,MRT2)
    call vecdot(vo(:,:,:,:,:,1),vo(:,:,:,:,:,1),betao,MRT2)
    
    beta(1) = betae(1) + betao(1) 
...
!!QUESTION: WHICH OF THE TWO DO I NEED? 

    !v(:,:,:,:,:,1) = const*v(:,:,:,:,:,1) !ORIGINAL -PL 
    ve(:,:,:,:,:,1) = const*ve(:,:,:,:,:,1) 
    vo(:,:,:,:,:,1) = const*vo(:,:,:,:,:,1) 

!!QUESTION CONT. OR DO I NEED THIS ONE?  

    !do isite=1,nvhalf 
    !    ve(:,isite,:,:,:,1) = const*ve(:,isite,:,:,:,1)
    !    vo(:,isite,:,:,:,1) = const*vo(:,isite,:,:,:,1)
    !enddo
...
! For use in Morgan's step 2a, define c = beta*e_1.
    ...
    c(2,1) = 0.0_KR2
    c2(:,1) = c(:,1)
!!!!! END ALL SHOULD BE IN MINRESDR5EIG


!!!!!END OF INVERTERS CODE
    ...
    !----------End of polynomial preconditioner initializations------
    ...
    !NEED TO USE PRECONDITIONER FILE HERE TO APPLY PRECONDITIONER WITH GAMMA MULT


    
!     call applyppgamma5( ve(:,:,:,:,:,j), vo(:,:,:,:,:,j),p,co,workEE,workOO,MRT,ieo,ibleo,gblclr,&
!                        kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
!                        bc,nms,ib,lbd,ldiv,lvbc)       
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Starting second pass just before newpoly" 
          endif
      endif 
      ...
      call newpoly( workEE, workOO, ve(:,:,:,:,:,j), vo(:,:,:,:,:,j),p,lorv,MRT,ieo,ibleo,gblclr,&
                    kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                    bc,nms,ib,lbd,ldiv,lvbc,1,MRT2,tEE,tOO)
       ...
       do ieo=1,2
        do ibleo=1,8
          call gammamult( workEE, workOO,tmpEE,tmpOO,5,ieo,ibleo)
        enddo
      enddo
    ...
    !!!!NEED A CALL TO APPLYPPGAMMA5 HERE TO TRANFORM we AND wo AND RETURN THEM
    ! y = p(A)z 
    
    !call applyppgamma5(workEE,workOO,p,co,we,wo,MRT,ieo,ibleo,gblclr,&
    !                   kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
    !                   bc,nms,ib,lbd,ldiv,lvbc)       
    ...
    !END NEED TO USE PRECONDITIONER FILE HERE TO APPLY PRECONDITIONER WITH GAMMA MULT
...
if (.true.) then
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==50 .OR. icycle==100 .OR. icycle==200 .OR. icycle==250 .OR. icycle==1000 .OR. icycle==2000) then
    ...
    !cvec=0.0_KR2
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just before least square problem" 
          endif
      endif 
...
!   endif !icycle, !end of test -TW 8/23/18
    ...
    !CHANGE #3 (ONLY NEED IF RIGHT PRECONDITIONED) 
    
    !NEED TO USE THE PRECONDITIONER FILE IN UPDATING THE SOLUTION 
   ...
   !endif
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just finished solving least squares problem" 
          endif
      endif 
   ...
   ! call applyppgamma5( ve(:,:,:,:,:,i), vo(:,:,:,:,:,i),p,co,TTmpe(:,:,:,:,:,i),TTmpo(:,:,:,:,:,i),MRT,ieo,ibleo,gblclr,&
   !                   kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
   !                   bc,nms,ib,lbd,ldiv,lvbc)       
   !enddo   
    ...
    do i=1,mdr
        do isite=1,nvhalf
            TTmpe(:,isite,:,:,:) = TTmpe(:,isite,:,:,:) + ym(i)*ve(:,isite,:,:,:,i)
            TTmpo(:,isite,:,:,:) = TTmpo(:,isite,:,:,:) + ym(i)*vo(:,isite,:,:,:,i) 
        enddo  
    enddo 
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just before newpoly on solution update" 
          endif
      endif 


    call newpoly( xTMPe(:,:,:,:,:), xTMPo(:,:,:,:,:), TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,MRT,ieo,ibleo,gblclr,&
                  kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                  bc,nms,ib,lbd,ldiv,lvbc,1,MRT2,tEE,tOO)
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just after newpoly on solution update" 
          endif
      endif 
  ...
  ! TTmpo = xo 
   ...
   !Update the solution and compute the residual norm
    do isite=1,nvhalf
        ...
        !TTmpe(:,isite,:,:,:)=TTmpe(:,isite,:,:,:)+ym(i)*ve(:,isite,:,:,:,i) !-PL 
        !TTmpo(:,isite,:,:,:)=TTmpo(:,isite,:,:,:)+ym(i)*vo(:,isite,:,:,:,i) !-PL 
        xe(:,isite,:,:,:)=xe(:,isite,:,:,:)+xTMPe(:,isite,:,:,:)
        xo(:,isite,:,:,:)=xo(:,isite,:,:,:)+xTMPo(:,isite,:,:,:)
    enddo !isite
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just updated solution a second time" 
          endif
      endif 
    
   
  !  call newpoly( TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,xe(:,:,:,:,:),xo(:,:,:,:,:),MRT,ieo,ibleo,gblclr,&
  !                   kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
  !                   bc,nms,ib,lbd,ldiv,lvbc)
    ...
    !END NEED TO USE THE PRECONDITIONER FILE IN UPDATING THE SOLUTION 
 ...
 if (.true.) then !residual calculation, does not determine stopping criteria -Travis          
   do i = 1,mdr+1
   tmp = 0.0_KR2
    do kk = 1,mdr
...
if (.true.) then !to get residual norm of linear equations -TW 11/14/18
        ...
        if (myid==0) then
         ...
         if (myid==0) then
           open(unit=34,file=trim(rwdir(myid+1))//"linearresidual.dat",status="old",action="write",&
           form="formatted",position="append")
           write(unit=34,fmt="(i4,a6,es19.12)")itercount," ",xrn(1)
           close(unit=34,status="keep")
         endif
endif
      residual_norm = rn2
   ...
   if (myid==0) then
    open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
         form="formatted",status="old",position="append")
    write(unit=8,fmt="(a48,i10,es17.10,es17.10,es17.10,es17.10)")"ppminresdr5EIG,itercount,rn,xrn,srvnorm,residual",&
         itercount,rn/normnum_hermetian,xrn(1)/normnum_hermetian,srvnorm/normnum_hermetian,residual_norm/normnum
    close(unit=8,status="keep")
   endif


   ! CHANGED BACK TO RELATIVE RESIDUAL 

   if( (srvnorm/normnum_hermetian.LT. resmax) .OR. (icycle > 500) ) then !changed from 2000 to 1 -TW 11/1/18                             
   ! if ( (xrn(1) < 1e-9_KR2 ) .OR. (icycle > 500)) then
...
if (.true.) then !to get residual norm of linear equations -TW 11/14/18
   ...
   if (myid==0) then
    ...
    ! ADDED THIS TO DO MORE CYCLES -PL

    if( (srvnorm/normnum_hermetian .LT. resmax) .OR. (icycle > 5000) ) then


   ! THIS WAS COMMENTED OUT BEFORE, CHANGING BACK -PL 
...
!check orthonormality of kdr evects here TW 8/10/18
 ...
 if (.false.) then !I cant tell if the evects are not orthonormal or if I have
 ! just not implemented this test rorrectly --Travis
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==100 .OR. icycle==1000 .OR. icycle==2000) then !print out orthog. of ritz vectors TW 8/10/18
   do kk = 1,kdr
   !tmpevect(:) = vr(:,kk)
    do jj = 1,kdr
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine newppgmresdr5EIG(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
                    iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                    lvbc,ib,lbd,iblv,MRT,MRT2)
    use shift
    !use preconditioning 
...
!------------------------------parameters for the polynomial-------------------
    ...
    ! mdr = ldr(1) 
    real(kind=KR2),     dimension(6,ntotal,4,2,8) :: TTmpe, TTmpo, xTMPe, xTMPo    
    real(kind=KR2),     dimension(2,nmaxGMRES)    :: harmrv, lorv
    real(kind=KR2),      dimension(6,ntotal,4,2,8) :: workEE, workOO 

!counter
    real(kind=KR2),     dimension(6,ntotal,4,2,8) :: xejunk, xojunk 
    integer(kind=KI),   dimension(2)                :: paramsGMRES
    integer(kind=KI)                                :: itermin2 
    !real(kind=KR2),     dimension(6,ntotal,4,2,8,mdr+1) :: vprimee, vprimeo
!------------------------------------------------------------------------------
    real(kind=KR2),     dimension(6,ntotal,4,2,8) :: tEE,tOO 
...
if (myid==0) then
 print *,'just entered newppgmresdr5EIG'
endif

if (myid == 0) then
   print *, 'the value of kappa in newppgmresdr5EIG is = ',kappa(1)
endif
    ...
    call gmres5EIGritz(rwdir,be,bo,xejunk,xojunk,paramsGMRES,u,GeeGooinv, &
                       iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                       lvbc,ib,lbd,iblv,MRT,MRT2,harmrv)

    if (myid==0) then 
        do i = 1,p 
            print*, 'rv:        ', harmrv(1,i), harmrv(2,i)  
        enddo 
    endif

    ! Call modlejacomp to perform modified Leja ordering on Harmonic ritz values
    
    call modlejacomp(harmrv,lorv,p)    
...
xrn = rn
 ...
 if (myid==0) then
  print *,'starting newppgmresdr5EIG while loop'
 endif
 ...
 ! do while(((rn(1)/rninit(1)) > 1e-9) .AND. (icycle <= 15))
 ! do while((xrn(1) > 1e-9_KR2) .AND. (icycle <= 500)) !changed to this for residual norm print out TW 11/14/18                    
  do while( (rn(1)/rninit(1)) > rtol .AND. (icycle <= 2000)) !changed back to checking based on rtol PL 12/14/21                    
 ! do while(icycle <=150)! to make sure if it works
    ! begin gmres(m) iterations
  ...
  !  endif
    ...
    do while(j <= m)
      ...
      ! For (right) Preconditioning, replace 
      ! >>> wv = M*g5*v with,  
      ! >>> wv = M*g5*p(M*g5)*v = M*g5 * work = M * temp  
      ! work = p(M g5) * v   ---> newpoly 
      ! temp = g5 * p(M g5) * v  ---> gammamult
      ! wv = M * temp  --- Hsingle, etc. 
...
! BS ********start comment out
  ...
  ! BS ****end comment out
    ...
    ! Replacing M * p(M) with just phi(M) 

     wve = 0.0_KR2 
     wvo = 0.0_KR2 

      call newphi( wve, wvo, ve(:,:,:,:,:,j), vo(:,:,:,:,:,j),p,lorv,MRT,ieo,ibleo,gblclr,&
                   kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
...
if (.false.) then
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==50 .OR. icycle==100 .OR. icycle==200 .OR. icycle==250 .OR. icycle==1000 .OR. icycle==2000) then
    ...
    if (icycle == 1 .or. icycle == 2) then 
        if (myid==0) then 
            print*, "Just before leastsquaredLAPACK, icycle = ", icycle 
        endif
    endif
    ...
    if (icycle == 1 .or. icycle == 2) then 
        if (myid==0) then 
            print*, "Just after leastsquaredLAPACK, icycle = ", icycle 
        endif
    endif
    ...
    do i=1,m 
        do icri = 1,5,2
            do jj = 1,nvhalf 
                TTmpe(icri  ,jj,:,:,:) = TTmpe(icri  ,jj,:,:,:) &
                                       + d(1,i)*ve(icri  ,jj,:,:,:,i) &
                                       - d(2,i)*ve(icri+1,jj,:,:,:,i)
                TTmpo(icri  ,jj,:,:,:) = TTmpo(icri  ,jj,:,:,:) &
                                       + d(1,i)*vo(icri  ,jj,:,:,:,i) &
                                       - d(2,i)*vo(icri+1,jj,:,:,:,i)
                TTmpe(icri+1,jj,:,:,:) = TTmpe(icri+1,jj,:,:,:) &
                                       + d(2,i)*ve(icri  ,jj,:,:,:,i) &
                                       + d(1,i)*ve(icri+1,jj,:,:,:,i)
                TTmpo(icri+1,jj,:,:,:) = TTmpo(icri+1,jj,:,:,:) &
                                       + d(2,i)*vo(icri  ,jj,:,:,:,i) &
                                       + d(1,i)*vo(icri+1,jj,:,:,:,i)
            enddo  
        enddo 
    ...
    !call newpoly( xTMPe(:,:,:,:,:), xTMPo(:,:,:,:,:), TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,MRT,ieo,ibleo,gblclr,&
    !              kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,bd,ldiv,lvbc,1,MRT2)

    call newpoly( xTMPe(:,:,:,:,:), xTMPo(:,:,:,:,:), TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,MRT,ieo,ibleo,gblclr,&
                  kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,1,MRT2,tEE,tOO)
    ...
    !if (myid==0) then 
    !    do jj=1,10
    !         print*, "TTmpe(1,jj,1,1,1) - xTMPe(1,jj,1,1,1): ", TTmpe(1,jj,1,1,1) - xTMPe(1,jj,1,1,1) 
    !         print*, "jj = ", jj
    !    enddo
    !endif 
    ...
    !if (myid==0) then 
    !    do jj=1,10
    !         print*, "TTmpe(2,jj,1,1,1) - xTMPe(2,jj,1,1,1): ", TTmpe(2,jj,1,1,1) - xTMPe(2,jj,1,1,1) 
    !         print*, "jj = ", jj
    !    enddo
    !endif 
    ...
    ! Setup and sovle linear equations problem
    ! x(:) = x(:) + v(:,1:m)*d(1:m)
    ! With a preconditioner, this becomes, 
    ! x(:) = x(:) + p(A)*v(:,1:m)*d(1:m), where p(A) is the preconditioner 
    do icri = 1,5,2 ! 6=nri*nc
        do jj = 1,nvhalf
          xe(icri  ,jj,:,:,:) = xe(icri  ,jj,:,:,:) + xTMPe(icri  ,jj,:,:,:) 
          xo(icri  ,jj,:,:,:) = xo(icri  ,jj,:,:,:) + xTMPo(icri  ,jj,:,:,:) 
          xe(icri+1,jj,:,:,:) = xe(icri+1,jj,:,:,:) + xTMPe(icri+1,jj,:,:,:) 
          xo(icri+1,jj,:,:,:) = xo(icri+1,jj,:,:,:) + xTMPo(icri+1,jj,:,:,:) 
        enddo
    enddo



    endif ! Falsed out old version of updating solution -PL 





    ! When preconditioning, this solution is y in Mp(M) y = b, and the 
    ! solution we need is x = p(M)y
...
!     if (myid==0) then
 ...
 !endif
      if (myid==0) then
       open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
            form="formatted",status="old",position="append")
        write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newppgmrs5EIG",icycle,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
        write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newppgmrs5EIG",icycle,"xrn=",xrn(1),"rnit=",rninit(1),xrn(1)/rninit(1)
  ...
  enddo
          ...
          if (myid==0) then !BS 
            do i =1,k 
                 write(*,fmt="(a17,i6,f19.11,a5,f19.11)") "evaluenewpp5EIG:",i,evalue(1,i),"",evalue(2,i)
 !BS               print * , 'evalue5EIG:',evalue
            enddo 
         endif
  ...
  if (myid==0) then
    open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
         form="formatted",status="old",position="append")
          write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newppgmrs5EIG",icycle-1,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
          write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newppgmrs5EIG",icycle-1,"xrn=",xrn(1),"rnit=",rninit(1),xrn(1)/rninit(1)
...
! Transforming preconditioned solution y, the solution to (Mg5)p(Mg5)y = b, into unpreconditioned solution  
! x = p(Mg5)y, the solution to (Mg5)x = b.  -PL  

TTmpe = xe  
TTmpo = xo 


call newpoly( xe(:,:,:,:,:), xo(:,:,:,:,:), TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,MRT,ieo,ibleo,gblclr,&
              kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,1,MRT2,tEE,tOO)
...
if(myid==0) then
print*, 'finished newppgmresdr5EIG'
endif



end subroutine newppgmresdr5EIG
!-------------------------------------------------------------------------------
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  subroutine newppminresdr5EIG(rwdir,be,bo,xe,xo,ldr,resmax,itermin,itercount,u,GeeGooinv, &
                      iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                      lvbc,ib,lbd,iblv,MRT,MRT2)
...
!------------------------------------------------------------------------------
    ...
    if (myid==0) then
        print *,'just entered newppminresdr5EIG'
    endif

    if (myid == 0) then
       print *, 'the value of kappa in newppminresdr5EIG is = ',kappa(1)
    endif
    ...
    xejunk(:6,:ntotal,:4,:2,:8) = 0.0_KR2   ! Returned by gmres5EIGritz but not needed 
    xojunk(:6,:ntotal,:4,:2,:8) = 0.0_KR2   ! Returned by gmres5EIGritz but not needed    
...
if (.true.) then !to get residual norm of linear equations -TW 11/14/18
   ...
   if (myid==0) then
    open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
         form="formatted",status="old",position="append")
    write(unit=8,fmt="(a40,i10,es17.10,es17.10,es17.10,es17.10)")"newppminresdr5EIG,itercount,rn,xrn,srvnorm,residual",&
         itercount,rn/normnum_hermetian,xrn(1)/normnum_hermetian,srvnorm/normnum_hermetian,residual_norm/normnum
    close(unit=8,status="keep")
   endif

   ! UNCOMMENTED THE ORIGINAL LINE AND CHANGED BACK -PL 
...
end subroutine newppminresdr5EIG
!- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine newdoubleppgmresdr5EIG(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
                    iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                    lvbc,ib,lbd,iblv,MRT,MRT2)
    use shift
    use preconditioning 
...
!------------------------------parameters for the polynomial-------------------
    ...
    real(kind=KR2),     dimension(6,ntotal,4,2,8)   :: trye,tryo,ye,yo
    integer(kind=KI)                                :: p, p1, p2

    ! mdr = ldr(1) 
    real(kind=KR2),     dimension(6,ntotal,4,2,8) :: TTmpe, TTmpo, xTMPe, xTMPo    
    real(kind=KR2),     dimension(2,nmaxGMRES)    :: harmrv1, harmrv2, lorv, lorv1, lorv2
    real(kind=KR2),      dimension(6,ntotal,4,2,8) :: workEE, workOO 
...
!------------------------------------------------------------------------------
    ...
    if (myid==0) then
        print *,'just entered newdoubleppgmresdr5EIG'
    endif

    if (myid == 0) then
        print *, 'the value of kappa in newdoubleppgmresdr5EIG is = ',kappa(1)
    endif
    ...
    p1 = 7 ! Polynomial degree 1 
    p2 = 7 ! Polynomial degree 2

    if (myid == 0) then
       print *, 'For first polynomial degree, using p1 = ',p1
       print *, 'For second polynomial degree, using p2 = ',p2 
    endif


    paramsGMRES1(1) = p1   ! dimension of Krylov subspace  
    ...
    paramsGMRES2(1) = p2   ! dimension of second Krylov subspace 
    paramsGMRES2(2) = 1    ! same as paramsGMRES1(2) 

    itermin2 = 10 

 
    !================================= CONSTRUCTION POLYNOMIAL PRECONDITIONERS ======================================
    !================================================================================================================
    ...
    call gmres5EIGritz(rwdir,be,bo,xejunk,xojunk,paramsGMRES1,u,GeeGooinv, &
                      iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                      lvbc,ib,lbd,iblv,MRT,MRT2,harmrv1)

    if (myid==0) then 
        do i = 1,p1 
            print*, 'rv1:        ', harmrv1(1,i), harmrv1(2,i)  
        enddo 
    endif

    ! Call modlejacomp to perform modified Leja ordering on Harmonic ritz values
    
    call modlejacomp(harmrv1,lorv1,p1)    
    ...
    call doublegmres5EIGritz(rwdir,be,bo,xejunk,xojunk,paramsGMRES2,lorv1,u,GeeGooinv, &
                      iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                      lvbc,ib,lbd,iblv,MRT,MRT2,harmrv2)

    if (myid==0) then 
        do i = 1,p2 
            print*, 'rv2:        ', harmrv2(1,i), harmrv2(2,i)  
        enddo 
    endif

    ! Call modlejacomp to perform modified Leja ordering on Harmonic ritz values
    
    call modlejacomp(harmrv2,lorv2,p2)    
    ...
    !=============================== END POLYNOMIAL PRECONDITIONER CONSTRUCTION =====================================
    !================================================================================================================
...
xrn = rn
 ...
 if (myid==0) then
  print *,'starting newdoubleppgmresdr5EIG while loop'
 endif
 ...
 ! do while(icycle <=150)! to make sure if it works
  ...
  !  endif
    ...
    do while(j <= m)
      ...
      ! Falsed out the original Mp(M) method 
...
! BS ********start comment out
  ...
  ! BS ****end comment out
      ...
      ! NEED TO APPLY GAMMA 5 FIRST SINCE THIS IS THE HERMITIAN VERSION 



      ! This is the double polynomial preconditioned system, i.e. phi_2(phi_1(M))y=b instead of a single 
      ! polynomial preconditioned system, phi_1(M)y = b    -PL  

      call newdoublephi( wve, wvo, ve(:,:,:,:,:,j), vo(:,:,:,:,:,j),p1,lorv1,p2,lorv2,MRT,ieo,ibleo,gblclr,&
                         kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                         bc,nms,ib,lbd,ldiv,lvbc,1,MRT2)
...
if (.false.) then
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==50 .OR. icycle==100 .OR. icycle==200 .OR. icycle==250 .OR. icycle==1000 .OR. icycle==2000) then
    ...
    endif ! Commented out old method of updating solution -PL
...
!     if (myid==0) then
 ...
 !endif
      if (myid==0) then
       open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
            form="formatted",status="old",position="append")
        write(unit=8,fmt="(a19,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrs5EIG",icycle,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
        write(unit=8,fmt="(a19,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrs5EIG",icycle,"xrn=",xrn(1),"rnit=",rninit(1),xrn(1)/rninit(1)
  ...
  enddo
          ...
          if (myid==0) then !BS 
            do i =1,k 
                 write(*,fmt="(a21,i6,f19.11,a5,f19.11)") "evaluenewdoublepp5EIG:",i,evalue(1,i),"",evalue(2,i)
 !BS               print * , 'evalue5EIG:',evalue
            enddo 
         endif
  ...
  if (myid==0) then
    open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
         form="formatted",status="old",position="append")
          write(unit=8,fmt="(a19,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrs5EIG",icycle-1,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
          write(unit=8,fmt="(a19,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrs5EIG",icycle-1,"xrn=",xrn(1),"rnit=",rninit(1),xrn(1)/rninit(1)
...
! NEED TO MAKE GAMMA 5 APPLICATIONS FIRST!!!! 


call newdoublepoly( xe(:,:,:,:,:), xo(:,:,:,:,:), TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p1,lorv1,p2,lorv2,MRT,ieo,ibleo,gblclr,&
                    kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,1,MRT2)



if(myid==0) then
print*, 'finished newdoubleppgmresdr5EIG'
endif



end subroutine newdoubleppgmresdr5EIG
!-------------------------------------------------------------------------------
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine newdoubleppgmresdrEIG(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
                    iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                    lvbc,ib,lbd,iblv,MRT,MRT2)
    use shift
    use preconditioning 
...
!------------------------------------------------------------------------------
    ...
    if (myid==0) then
        print *,'just entered newdoubleppgmresdrEIG'
    endif

    if (myid == 0) then
        print *, 'the value of kappa in newdoubleppgmresdrEIG is = ',kappa(1)
    endif
...
xrn = rn
 ...
 if (myid==0) then
  print *,'starting newdoubleppgmresdrEIG while loop'
 endif
 ...
 ! do while(icycle <=150)! to make sure if it works
  ...
  !  endif
    ...
    do while(j <= m)
      ...
      ! %%%%%%%%%%%%%%%%%%% THIS IS THE ORIGINAL SINGLE POLYNOMIAL METHOD FALSED OUT %%%%%%%%%%%%%%%%%%%%%%%%%%%
      ! Falsed out the original Mp(M) method 
...
if (.false.) then
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==50 .OR. icycle==100 .OR. icycle==200 .OR. icycle==250 .OR. icycle==1000 .OR. icycle==2000) then
    ...
    ! %%%%%%%%%%%%%%%%%%%%%%%% OLD SINGLE POLYNOMIAL METHOD FALSED OUT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ! Commented out the old method of apply poly at each cycle -PL  
    if(.false.) then 
...
!BS  4/9/016 to calculate residual using directmethod
      ...
      ! Gamma5 not needed in non-hermitian version. 
      !do ieo=1,2
      !  do ibleo=1,8
      !    call gammamult( xe(:,:,:,:,:), xo(:,:,:,:,:),xtmpEE,xtmpOO,5,ieo,ibleo)
      !  enddo
...
!     if (myid==0) then
 ...
 !endif
      if (myid==0) then
       open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
            form="formatted",status="old",position="append")
        write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrsEIG",icycle,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
        write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrsEIG",icycle,"xrn=",xrn(1),"rnit=",rninit(1),xrn(1)/rninit(1)
  ...
  enddo
          ...
          if (myid==0) then !BS 
            do i =1,k 
                 write(*,fmt="(a17,i6,f19.11,a5,f19.11)") "evaluenewdoubleppEIG:",i,evalue(1,i),"",evalue(2,i)
 !BS               print * , 'evalue5EIG:',evalue
            enddo 
         endif
  ...
  if (myid==0) then
    open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
         form="formatted",status="old",position="append")
          write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoublewppgmrsEIG",icycle-1,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
          write(unit=8,fmt="(a14,i4,a6,es11.4,a6,es11.4,es17.10)") "newdoubleppgmrsEIG",icycle-1,"xrn=",xrn(1),"rnit=",rninit(1),xrn(1)/rninit(1)
...
call newdoublepoly( xe(:,:,:,:,:), xo(:,:,:,:,:), TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p1,lorv1,p2,lorv2,MRT,ieo,ibleo,gblclr,&
                    kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)



if(myid==0) then
print*, 'finished newdoubleppgmresdrEIG'
endif



end subroutine newdoubleppgmresdrEIG
!-------------------------------------------------------------------------------

...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine gmresdr5EIG(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
    ...
    real(kind=KR2),   dimension(6,ntotal,4,2,8) :: tmpOO, tmpEE !BS
...
! ****BS comment out until this line *******
     ...
     gblclr = 2
      ...
      call Hsingle(wvo,u,tmpOO,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,
&
                   nms,lvbc,ib,lbd,iblv,MRT)
      ! Next build H_eo * v_o.
      gblclr = 1
      call Hsingle(wve,u,tmpEE,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,
&
                   nms,lvbc,ib,lbd,iblv,MRT)
         ...
         rn = betashift(1,1)
          ...
          ! if (j >= nDR) then 
             if (myid==0) then
               open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
                     form="formatted",status="old",position="append")
                 write(unit=8,fmt=*) "gmresdrproject-gdr",mvp,rn/rinit
         ...
         rn = betashift(1,1)
          ...
          ! if (j >= nDR) then 
             if (myid==0) then
               open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
                     form="formatted",status="old",position="append")
                 write(unit=8,fmt=*) "gmresdrproject-gdr",itercount,rn/rinit
                  write(unit=8,fmt=*) "gmresdrproject-gdr",itercount,gdr(mvp,1)/rinit!,gdr(mvp,2), gdr(mvp,3), gdr(mvp,4)
                 !print *, betashift(1,1), betashift(1,2), betashift(1,3)
                close(unit=8,status="keep")
             endif
         ...
         rn = betashift(1,1)
          ...
          ! if (j >= nDR) then 
             if (myid==0) then
                open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
                     form="formatted",status="old",position="append")
                  write(unit=8,fmt=*) "gmresdrproject-rn",itercount,rn/rinit
                  write(unit=8,fmt=*) "gmresdrproject-gdr",itercount,gdr(mvp,1)/rinit!,gdr(mvp,2), gdr(mvp,3), gdr(mvp,4)
                 !print *, betashift(1,1), betashift(1,2), betashift(1,3)
                close(unit=8,status="keep")
             endif
         ...
         rn = betashift(1,1)
          ...
          ! if (j >= nDR) then 
             if (myid==0) then
                open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
                     form="formatted",status="old",position="append")
                  write(unit=8,fmt=*) "gmresdrproject-rn",mvp,rn/rinit
!                  write(unit=8,fmt=*) "gmresdrproject-gdr",itercount,gdr(mvp,1)/rinit!,gdr(mvp,2), gdr(mvp,3), gdr(mvp,4)
                 !print *, betashift(1,1), betashift(1,2), betashift(1,3)
                close(unit=8,status="keep")
             endif

...
!   real(kind=KR2),   dimension(6,ntotal,4,2,8)             :: xb
 ...
 ! print *,'k=',k
  ...
  rn = rninit
  vn = rn
  xrn(1) = rninit(1) !this value will be output to the CFGSPROPS file
                     !for each cycle because we are not calculating 
                     !the resiual for the linear eqns to keep mvp's
                     !down for data collection - TW 9/7/18
  ...
  ! begin cycles
  do while(((rn(1)/rninit(1)) > 1e-100_KR2) .AND. (icycle <= 300))!becomes zero in 29 cycles
...
! MRT2 is MPIDBLETYPE.
    ...
    enddo   !ibleo
            ...
            print *,"inside matrixmultiply matmul(6,128,4,2,8,5)",matmul(6,128,4,2,8,5)
            print *,"inside matrixmultiply matmul(6,128,4,2,7,5)",matmul(6,128,4,2,7,5)

...
!============================ Constructing Polynomial ===============================
    ...
    call gmresEIGritz(rwdir,z2e,z2o,xejunk,xojunk,paramsGMRES,resmax,itermin,itercount,u,GeeGooinv, &
...
psignal = 0!the switch used to determine whether calculate the coeffecients for            !other right hands
 ...
 do irhs=1,nrhs !200
  ...
  if (myid==0) then 
   ...
   if (myid==0) then 
        print*, 'Line 2811'
        print*, 'z2e(1,1,1,1,1)', z2e(1,1,1,1,1) 
        print*, 'z2o(1,1,1,1,1)', z2o(1,1,1,1,1)
        print*, 'z2e(2,1,1,1,1)', z2e(2,1,1,1,1) 
        print*, 'z2o(2,1,1,1,1)', z2o(2,1,1,1,1)  
   endif 
...
!******************************end**************************!
    ...
    !call minresdr5EIG(rwdir,z2e,z2o,xeTMP(:,:,:,:,:,1),xoTMP(:,:,:,:,:,1),nGMRES,resmax,itermin,itercount,u,GeeGooinv, &
    !                  iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
    !                  lvbc,ib,lbd,iblv,MRT,MRT2)
   ...
   if(myid==0) then 
       print*, 'Line 2857' 
       print*, 'gxe(1,1,1,1,1)', gxe(1,1,1,1,1) 
       print*, 'gxo(1,1,1,1,1)', gxo(1,1,1,1,1)
       print*, 'gxe(2,1,1,1,1)', gxe(2,1,1,1,1)
       print*, 'gxo(2,1,1,1,1)', gxo(2,1,1,1,1)  
   endif 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!7order!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (.true.) then 
        ...
        call printferm(subeTMP,suboTMP,rwdir,oldppfile,myid,iblv,vecblinv,MRT2)!BS printing 2/4/2016
...
!============================ Constructing Polynomial ===============================
    ...
    call gmresEIGritz(rwdir,vprimee,vprimeo,xejunk,xojunk,paramsGMRES,resmax,itermin,itercount,u,GeeGooinv, &
                      iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                      lvbc,ib,lbd,iblv,MRT,MRT2,rv)
...
!========================== Applying the Polynomial ================================
    ...
    call newapplypp(vprimee,vprimeo,p,lorv,subeTMP,suboTMP,MRT, &
                    ieo,ibleo,gblclr,kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv, &
                    bc,nms,ib,lbd,ldiv,lvbc,0,MRT2)
    ...
    if (.true.) then 
        ...
        if( finished .ne. 1) then
           ...
           !endif
               ...
               if (myid==0) then
                   ...
                   if (.not. exists) then
                      open(unit=101,file=trim(rwdir(myid+1))//trim(newppfile)//".LOG",status="new",     &
              action="write",form="formatted")
        ...
        call printferm(subeTMP,suboTMP,rwdir,newppfile,myid,iblv,vecblinv,MRT2)!BS printing 2/4/2016

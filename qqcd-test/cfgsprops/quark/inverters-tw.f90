...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine gmresdr5EIG(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
    ...
    real(kind=KR2),   dimension(6,ntotal,4,2,8) :: tmpOO, tmpEE,xtmpEE,xtmpOO !BS
    ...
    integer(kind=KI) :: iblock, isite, idirac,icolorir, site, icolorr, irow,gblclr,exists 
...
!BS      if (myid==0) then
 ...
 ! do while(((rn(1)/rninit(1)) > 1e-450_KR2) .AND. (icycle <= 200) )!becomes zero in 29 cycles, TW 11/25/17
 ! do while(((rn(1)/rninit(1)) > 1e-9) .AND. (icycle <= 15))
  !do while((resi > 1e-9) .AND. (icycle <= 15))
  do while(icycle <=1000)! to make sure if it works
    ...
    ! Compute Residual Norm of Eigenvectors of hh matrix
       do i=1,k
         ...
         rna(i) = sqrt((tmp1(1)*tmp1(1)) + (h(1,m+1,m)*h(1,m+1,m) + h(2,m+1,m)*h(2,m+1,m)) &
                *(g(1,m,i)*g(1,m,i) + g(2,m,i)*g(2,m,i)))! Make sure if it is
!eigenvector norm or linear equation residual norm, changed + to *, changed
!brackets, originally (h(1,m+1,m)*h(1,m+1,m))+(h(2,m+1,m)*h(2,m+1,m)) and same
!with g
       enddo
   ! enddo
...
!end TW 11/20/17
   ...
   ! gg(:,:,k+1) = srv TW 11/20/17
    ...
    i = 1
    do while (rna(i) < 1E-8)!changed from 1E-12 to 1.5 3/15/17 TW
      hcnew(:,i+1:k+1,i) = 0.0_KR2 !changed to hcnew from hnew TW 3/23/17
      i = i + 1
    enddo
    ...
    do j=1,k
          ...
          do id = 1,4
            ...
            do i = 1,nvhalf
              ...
              do kk=1,m
                ...
                enddo
              enddo
              !Added to get evecs of any cycle, just change the cycle number
              !below TW 3/21/17
             ! if (icycle==3000) then
              !  ove(:,i,id,ieo,ibleo,j) = vte(:)
               ! ovo(:,i,id,ieo,ibleo,j) = vto(:)
             ! elseif (icycle/=3000) then
     ...
     ! elseif (icycle/=3000) then
        evalue(:,j) = th(:,ind(j))
           ! if (myid==0) then
            !  print *, 'evalue5EIG:',j,'j:',evalue(1,j)
           ! endif
     ! endif
    enddo
       !     do while(j <= m)
...
!BS residual for linear equation ends here
   ...
   enddo !i
    ...
    i = 1
    do while (rna(i) < 1E-12) !whytet 1/26/17
      hcnew(:,i+1:k+1,i) = 0.0_KR2 !changed to hcnew from hnew
      i = i + 1
    enddo
...
! Compute r=phi-M*x and v=r/|r| and beta=|r|.
  ...
  !    print *, "i,j, lsmat(:,i,j)=", i-1,j-1, lsmat(:,i-1,j-1) !uncommented TW
     enddo!j
    enddo!i
...
! eigenvalues for the large matrix.  (Not always accurate approximations.)
      ...
      call qgenerator(nDR,ilo,ihi,z,tau,work)
      ischur = 1
      if(myid==0) then !TW 3/14/17
        print *, 'Entering evalues of ppgmresdr'
      endif
      call evalues(ischur,nDR,ilo,ihi,hc2,w,z,work)

...
!------------------------------parameters for the polynomial-------------------
    ...
    ! mdr = ldr(1) 
    real(kind=KR2),     dimension(6,ntotal,4,2,8,ldr(1)+1) :: TTmpe, TTmpo    
...
!------------------------------------------------------------------------------
    ...
    !call vecdot(be,be,tv1,MRT2) !ORIGINAL -PL
    !call vecdot(bo,bo,tv2,MRT2) !ORIGINAL -PL 
    !tv = tv1 + tv2 !ORIGINAL -PL
    !normnum=sqrt(tv(1)) !ORIGINAL -PL 
    ...
    !call vecdot(r0e,r0e,tv1,MRT2) !ORIGINAL -PL 
    !call vecdot(r0o,r0o,tv2,MRT2) !ORIGINAL -PL
    !tv = tv1 + tv2 !ORIGINAL - PL
    !normnum_hermetian=sqrt(tv(1)) !ORIGINAL - PL
    !xrn(1) = sqrt(tv(1)) !ORIGINAL - PL 
                          ...
                          ! for data collection
    !rn2 = sqrt(tv(1))     !ORIGINAL - PL 
                          ! same as above

    !if (myid == 0) then !ORIGINAL - PL 
    !   print *, 'value of normnum_hermitian = ',normnum_hermetian !ORIGINAL -PL
    !endif !ORIGINAL -PL 

    !const=1.0_KR2/normnum_hermetian !ORIGINAL -PL 
    ...
    !do isite=1,nvhalf    !ORIGINAL  !ORIGINAL -PL 
    ! vc(:,isite,:,:,:,1)=const*r0(:,isite,:,:,:) !ORIGINAL
    !  ve(:,isite,:,:,:,1)=const*r0e(:,isite,:,:,:) !ORIGINAL -PL 
    !  vo(:,isite,:,:,:,1)=const*r0o(:,isite,:,:,:) !ORIGINAL -PL 
    !enddo !isite !ORIGINAL  !ORIGINAL -PL 
...
!!!!!!!!!!This will need a call to the function that returns y = P(A)*r (applyppgamma5)
    ...
    !    v(:,k,:,:,:,1) = y(:,k,:,:,:,1) !ORIGINAL -PL 
        ve(:,k,:,:,:,1) = ye(:,k,:,:,:) ! Use y=P(A)*r to generate V_(m+1)&Hbar_m
        vo(:,k,:,:,:,1) = yo(:,k,:,:,:) 
    enddo!k
  ...
  if (icycle==1 .OR. icycle==2 .OR. icycle==10 .OR. icycle==50 .OR. icycle==100 .OR. icycle==200 .OR. icycle==250 .OR. icycle==1000 .OR. icycle==2000) then
   ...
   do i=1,mdr
    do isite=1,nvhalf
     ...
     !xe(:,isite,:,:,:)=xe(:,isite,:,:,:)+ym(i)*ve(:,isite,:,:,:,i) -PL 
     !xo(:,isite,:,:,:)=xo(:,isite,:,:,:)+ym(i)*vo(:,isite,:,:,:,i) -PL 
     xe(:,isite,:,:,:)=xe(:,isite,:,:,:)+ym(i)*TTmpe(:,isite,:,:,:,i)
     xo(:,isite,:,:,:)=xo(:,isite,:,:,:)+ym(i)*TTmpo(:,isite,:,:,:,i)
    enddo !isite
   enddo !i
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 subroutine newapplypp(trye,tryo,deg,rv,ye,yo,MRT,ieo,ibleo,gblclr,&
                       kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                       bc,nms,ib,lbd,ldiv,lvbc)

!===========================================================================
!   A subroutine to implement the GMRES polynomial pi(M) by applying
!   it to a vector z and returning it as y (i.e. y = pi(M)z).
!===========================================================================
!   INPUT:
!
!       trye    = Even vector z to apply p(M) to
!       tryo    = Odd vector z to apply p(M) to
!       deg     = Degree of polynomial is deg-1
!       rv      = Harmonic Ritz values for p(M)
!       MRT     = MPIREALTYPE

!---------------------------------------------------------------------------
!   OUTPUT:
!
!       ye      = trye with GMRES polynomial pi(M)
!                 applied to it. (i.e. ye = pi(M) * trye)
!       yo      = tryo with GMRES polynomial pi(M)
!                 applied to it. (i.e. yo = pi(M) * tryo)
!---------------------------------------------------------------------------
!   FUTURE VERSIONS:
!
...
real(kind=KR),                         dimension(6,nvhalf,4,2,8)    :: re,ro
integer(kind=KI),      intent(in)                                   :: deg
real(kind=KR),                         dimension(6,nvhalf,4,2,8)    :: trye,tryo
real(kind=KR2),        intent(in),     dimension(2,deg)             :: rv
integer(kind=KI),      intent(in)                                   :: MRT
real(kind=KR2),                        dimension(6,nvhalf,4,2,8)    :: tmpEE,tmpOO,&
                                                                       we,wo
integer(kind=KI)                                                    :: ieo,ibleo,&
                                                                       gblclr,i,k,&
                                                                       icri
...
real(kind=KR2),                         dimension(6,nvhalf,4,2,8)   :: polye
real(kind=KR2),                         dimension(6,nvhalf,4,2,8)   :: polyo
real(kind=KR2),                         dimension(6,nvhalf,4,2,8)   :: prode 
real(kind=KR2),                         dimension(6,nvhalf,4,2,8)   :: prodo 
real(kind=KR2),                         dimension(6,nvhalf,4,2,8)   :: tmpe
real(kind=KR2),                         dimension(6,nvhalf,4,2,8)   :: tmpo 
real(kind=KR2)                                                      :: a,b
...
do while( i <= deg-1 ) 
    if( rv(2,i) .eq. 0 ) then 
        ...
        !write(*,*) "In if" 
        polye = polye + (1/rv(1,i))*prode
        polyo = polyo + (1/rv(1,i))*prodo
        ...
        prode = prode - (1/rv(1,i))*we
        prodo = prodo - (1/rv(1,i))*wo
        i = i + 1
    else
        ...
        ye = ye + (1/(a**2 + b**2)) * tmpe 
        yo = yo + (1/(a**2 + b**2)) * tmpo 
...
if( rv(2,i) .eq. 0 ) then 
! I don't think the loops are necessary. It doesn't have the complex part of 
! the coefficient issue that there was before 
    do icri = 1,5,2
        do k = 1,nvhalf 
            ye(icri,k,:,:,:)   = polye(icri,k,:,:,:) + &
                                    (1/rv(1,i))*prode(icri,k,:,:,:) 
            ye(icri+1,k,:,:,:) = polyo(icri+1,k,:,:,:) + & 
                                    (1/rv(1,i))*prodo(icri+1,k,:,:,:) 
            yo(icri,k,:,:,:)   = polyo(icri,k,:,:,:) + & 
                                    (1/rv(1,i))*prodo(icri,k,:,:,:) 
            yo(icri+1,k,:,:,:) = polyo(icri+1,k,:,:,:) + & 
                                    (1/rv(1,i))*prodo(icri+1,k,:,:,:) 
        enddo !k 
    enddo !icri 
endif 
...
!endif
        if (myid==0) then 
             print*, "Printing gmres5EIGritz residual for newxipolygenerator" 
        endif

...
! Define access to subroutines.
    public  :: bicgstab, m3r, cgne, qmr, qmrgam5, gmres, ppgmresproject, &
               ...
               applyppgamma5, powerbasis, newapplypp, modlejacomp,gmresEIGritz, &
               gmres5EIGritz, newppgmresdr5EIG, newppminresdr5EIG 
...
!!!!!!!!!!This will need a call to the function that returns y = P(A)*r (applyppgamma5)
    ...
    call gmres5EIGritz(rwdir,be,bo,xe,xo,paramsGMRES,resmax,itermin,itercount,u,GeeGooinv, &
                       iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
    ...
    !call newapplypp(trye,tryo,p,lorv,ye,yo,MRT,ieo,ibleo,gblclr,&
    !                kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
    !                bc,nms,ib,lbd,ldiv,lvbc)
...
!                        bc,nms,ib,lbd,ldiv,lvbc)       
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Starting second pass just before newapplypp" 
          endif
      endif 
      ...
      call newapplypp( ve(:,:,:,:,:,j), vo(:,:,:,:,:,j),p,lorv,workEE,workOO,MRT,ieo,ibleo,gblclr,&
                       kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
...
!   endif !icycle, !end of test -TW 8/23/18
   ...
   !enddo   
    ...
    enddo 
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just before newapplypp on solution update" 
          endif
      endif 


    call newapplypp( TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,xTMPe(:,:,:,:,:),xTMPo(:,:,:,:,:),MRT,ieo,ibleo,gblclr,&
                     kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                     bc,nms,ib,lbd,ldiv,lvbc,1,MRT2)
      ...
      if (icycle == 2) then 
          if(myid==0) then
              print*, "Second pass just after newapplypp on solution update" 
          endif
      endif 
  ...
  !  call newapplypp( TTmpe(:,:,:,:,:), TTmpo(:,:,:,:,:),p,lorv,xe(:,:,:,:,:),xo(:,:,:,:,:),MRT,ieo,ibleo,gblclr,&
  !                   kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
  !                   bc,nms,ib,lbd,ldiv,lvbc)
...
!===========================================================================
!   A subroutine to implement the polynomial preconditioner p(A) by applying
!   it to a vector z and returning it (i.e. y = p(A)z).
!===========================================================================
!   INPUT:
!
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 subroutine newapplypp(vIne,vIno,deg,hRV,polye,polyo,MRT,ieo,ibleo,gblclr,&
                       kappa,u,coact,vecbl,vecblinv,idag,myid,nn,iblv,&
                       bc,nms,ib,lbd,ldiv,lvbc,herm,MRT2)

!===========================================================================
!   A subroutine to implement the GMRES polynomial preconditioner p(M) 
!   by applying it to a vector z and returning it as y (i.e. y = p(M)z).
!
!===========================================================================
!   INPUT:
!
!       vIne    = Even vector z to apply p(M) to
!       vIno    = Odd vector z to apply p(M) to
...
!real(kind=KR),         intent(out),    dimension(6,nvhalf,4,2,8)    :: ye,yo
!real(kind=KR),                         dimension(6,ntotal,4,2,8)    :: re,ro
...
real(kind=KR2),        intent(out),     dimension(:,:,:,:,:)        :: polye
real(kind=KR2),        intent(out),     dimension(:,:,:,:,:)        :: polyo
        ...
        do icri = 1,5,2
            do k = 1,nvhalf 
                polye(icri  ,k,:,:,:) = polye(icri  ,k,:,:,:) &
                                      + (1/harv(1,i))*prode(icri  ,k,:,:,:) &
                                      - (1/harv(2,i))*prode(icri+1,k,:,:,:)  
                polyo(icri  ,k,:,:,:) = polyo(icri  ,k,:,:,:) & 
                                      + (1/harv(1,i))*prodo(icri  ,k,:,:,:) & 
                                      - (1/harv(2,i))*prodo(icri+1,k,:,:,:)  
                polye(icri+1,k,:,:,:) = polye(icri+1,k,:,:,:)   & 
                                      + (1/harv(2,i))*prode(icri  ,k,:,:,:) &    
                                      + (1/harv(1,i))*prode(icri+1,k,:,:,:) 
                polyo(icri+1,k,:,:,:) = polyo(icri+1,k,:,:,:) & 
                                      + (1/harv(2,i))*prodo(icri  ,k,:,:,:) & 
                                      + (1/harv(1,i))*prodo(icri+1,k,:,:,:)  
            enddo !k 
        enddo !icri 
     endif     
        ...
        do icri = 1,5,2
            do k = 1,nvhalf 
                prode(icri  ,k,:,:,:) = prode(icri  ,k,:,:,:) &
                                      - (1/harv(1,i))*we(icri  ,k,:,:,:) &
                                      + (1/harv(2,i))*we(icri+1,k,:,:,:)  
                prodo(icri  ,k,:,:,:) = prodo(icri  ,k,:,:,:) & 
                                      - (1/harv(1,i))*wo(icri  ,k,:,:,:) & 
                                      + (1/harv(2,i))*wo(icri+1,k,:,:,:)  
                prode(icri+1,k,:,:,:) = prode(icri+1,k,:,:,:)   & 
                                      - (1/harv(2,i))*we(icri  ,k,:,:,:) &    
                                      - (1/harv(1,i))*we(icri+1,k,:,:,:) 
                prodo(icri+1,k,:,:,:) = prodo(icri+1,k,:,:,:) & 
                                      - (1/harv(2,i))*wo(icri  ,k,:,:,:) & 
                                      - (1/harv(1,i))*wo(icri+1,k,:,:,:)  
            enddo !k 
        enddo !icri 
    endif 
...
if (herm==1) then 
    do icri = 1,5,2
        do k = 1,nvhalf 
            polye(icri  ,k,:,:,:) = polye(icri  ,k,:,:,:) + (1/harv(1,deg))*prode(icri  ,k,:,:,:) 
            polyo(icri  ,k,:,:,:) = polyo(icri  ,k,:,:,:) + (1/harv(1,deg))*prodo(icri  ,k,:,:,:) 
            polye(icri+1,k,:,:,:) = polye(icri+1,k,:,:,:) + (1/harv(1,deg))*prode(icri+1,k,:,:,:) 
            polyo(icri+1,k,:,:,:) = polyo(icri+1,k,:,:,:) + (1/harv(1,deg))*prodo(icri+1,k,:,:,:) 
        enddo
    enddo
else
    do icri = 1,5,2
        do k = 1,nvhalf 
            polye(icri  ,k,:,:,:) = polye(icri  ,k,:,:,:) &
                               + (1/harv(1,deg))*prode(icri  ,k,:,:,:) &
                               - (1/harv(2,deg))*prode(icri+1,k,:,:,:)  
            polyo(icri  ,k,:,:,:) = polyo(icri  ,k,:,:,:) & 
                               + (1/harv(1,deg))*prodo(icri  ,k,:,:,:) & 
                               - (1/harv(2,deg))*prodo(icri+1,k,:,:,:)  
            polye(icri+1,k,:,:,:) = polye(icri+1,k,:,:,:)   & 
                               + (1/harv(2,deg))*prode(icri  ,k,:,:,:) &    
                               + (1/harv(1,deg))*prode(icri+1,k,:,:,:) 
            polyo(icri+1,k,:,:,:) = polyo(icri+1,k,:,:,:) & 
                               + (1/harv(2,deg))*prodo(icri  ,k,:,:,:) & 
                               + (1/harv(1,deg))*prodo(icri+1,k,:,:,:)  
        enddo !k 
    enddo !icri 
endif 
...
call vecdot(diffe(:,:,:,:,:),diffe(:,:,:,:,:),tmpe,MRT2) 
call vecdot(diffo(:,:,:,:,:),diffo(:,:,:,:,:),tmpo,MRT2) 

tmp10 = tmpe + tmpo 

if(myid==0) then 
    print*, 'norm(poly - vIn) = ', sqrt(tmp10(1)) 
endif 

endsubroutine newapplypp
 !!!!!!
...
! I don't think I need these for anything -PL 
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine gmresEIGritz(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
                    iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
...
!   with gamma5 multiplication removed and hardcoded for a single cycle 
!   -PL 
...
if (.true.) then ! Ignore residual calculation 
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 subroutine gmres5EIGritz(rwdir,be,bo,xe,xo,GMRES,rtol,itermin,itercount,u,GeeGooinv, &
                    iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                    lvbc,ib,lbd,iblv,MRT,MRT2,hrv)
...
!   An implementation of gmres for obtaining ritz values to be used in 
!   polynomial preconditioning. Code copied and pasted from gmresdr5EIG 
!   and hardcoded for a single cycle 
!   -PL 
!   Some things could be removed near the end since there is no restart and 
!   deflation, so it's wasted calculation. Single cycle implemented by 
!   simply removing while loop over main algorithm (i.e. part of algorithm 
...
!     if (myid==0) then
 ...
 !endif
      if (myid==0) then
       open(unit=8,file=trim(rwdir(myid+1))//"CFGSPROPS.LOG",action="write", &
            form="formatted",status="old",position="append")
        write(unit=8,fmt="(a9,i4,a6,es11.4,a6,es11.4,es17.10)") "gmres5EIGritz",icycle,"rn=",rn(1),"rnit=",rninit(1),rn(1)/rninit(1)
  ...
  !enddo !!END OF MAIN ALGORITHM !Removed while loop so only a single cycle is performed -PL 1/12/21
          ...
          if (myid==0) then !BS 
            do i =1,k 
                 write(*,fmt="(a12,i6,f19.11,a5,f19.11)") "evalue5EIGritz:",i,evalue(1,i),"",evalue(2,i)
 !BS               print * , 'evalue5EIG:',evalue
            enddo 
         endif
...
if (myid == 0) then
   print *, 'the value of kappa in newppgmresdr5EIG is = ',kappa(1)
    ...
    call gmres5EIGritz(rwdir,be,bo,xejunk,xojunk,paramsGMRES,1.0E-8_KR2,itermin2,itercount,u,GeeGooinv, &
                       iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
